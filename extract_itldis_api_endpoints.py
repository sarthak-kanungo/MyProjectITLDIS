...
def find_user_defined_files(controller_file, dependencies, content):
    ...
    import_pattern = r'import\s+com\.i4o\.dms\.itldis\.([^;]+);'
    ...
    for imp in imports:
        ...
        if is_user_defined(class_name):
            ...
            if len(parts) >= 2:
                ...
                potential_path = f"ITLDIS-BACKEND/src/main/java/com/i4o/dms/itldis/{package_path}/{file_name}"
...
def extract_api_endpoints():
    """Extract all API endpoints from ITLDIS backend"""
    
    base_path = Path("ITLDIS-BACKEND/src/main/java")
    controllers = []
    ...
    for controller_file in controllers:
        try:
            ...
            package_match = re.search(r'package\s+(com\.i4o\.dms\.itldis[^;]+);', content)
...
def create_excel(endpoints):
    ...
    # Save file to ITLDIS folder
    output_file = "ITLDIS/ITLDIS_API_Endpoints_Detailed.xlsx"
    
    # Create ITLDIS folder if it doesn't exist
    os.makedirs("ITLDIS", exist_ok=True)
    ...
    try:
        ...
        temp_file = f"ITLDIS/ITLDIS_API_Endpoints_Detailed_{timestamp}.xlsx"
        wb.save(temp_file)
...
if __name__ == "__main__":
    print("Extracting ITLDIS API endpoints with database objects (tables, stored procedures, etc.)...")

#!/usr/bin/env python3
"""
Script to convert KUBOTA ACCPAC Module UML Class Diagram from markdown file to JPEG image
"""
...
def mermaid_to_jpeg(mermaid_code, output_path, title="ACCPAC Module UML Diagram", width=16000, height=12000):
...
{cleaned_code}
    ...
    with sync_playwright() as p:
        ...
        # Set viewport to reasonable size for full capture
        max_viewport_width = 16000
        max_viewport_height = 12000
        final_width = min(int(svg_width * 1.1), max_viewport_width) if svg_width else max_viewport_width
        final_height = min(int(svg_height * 1.1), max_viewport_height) if svg_height else max_viewport_height
        
        print(f"   Setting viewport for full capture: {final_width}x{final_height}")
        page.set_viewport_size({'width': final_width, 'height': final_height})
        page.wait_for_timeout(5000)
        ...
        # Use SVG element screenshot - captures the complete SVG element
        svg_element = page.query_selector('svg')
        if svg_element:
            try:
                screenshot_bytes = svg_element.screenshot(type='png', timeout=180000)
                print(f"   Full SVG element captured - complete diagram")
            except Exception as e:
                error_msg = str(e)[:150]
                print(f"   SVG screenshot failed: {error_msg}, trying full page...")
                # Fallback to full page
                page.set_viewport_size({'width': 14000, 'height': 10000})
                page.wait_for_timeout(3000)
                screenshot_bytes = page.screenshot(full_page=True, type='png', timeout=180000)
...
def main():
    """Main function to convert ACCPAC Module UML diagram"""
    ...
    markdown_file = script_dir / "ACCPAC-MODULE-UML-DIAGRAM.md"
    ...
    output_filename = "ACCPAC-Module-UML-Class-Diagram.png"
    ...
    try:
        ...
        old_jpg = output_dir / "ACCPAC-Module-UML-Class-Diagram.jpg"
        if old_jpg.exists():
            old_jpg.unlink()
            print(f"   Deleted old JPG file: {old_jpg}")
        
        mermaid_to_jpeg(diagram, output_path, "ACCPAC Module UML Class Diagram", width=16000, height=12000)

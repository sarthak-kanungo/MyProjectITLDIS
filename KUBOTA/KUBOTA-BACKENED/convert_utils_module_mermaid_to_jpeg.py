#!/usr/bin/env python3
"""
Script to convert Utils Module Mermaid diagrams from markdown file to JPEG images
"""
...
def extract_mermaid_diagrams(markdown_file):
    ...
    for i, line in enumerate(lines):
        if line.startswith('## '):
            # Extract title number and description
            title_text = line.replace('## ', '').strip()
            # Remove "Flow" suffix if present and clean up
            if title_text.endswith(' Flow'):
...
def main():
    ...
    markdown_file = script_dir / "UTILS_MODULE_SEQUENCE_DIAGRAMS.md"
    output_dir = script_dir / "UtilsFlow"

#!/usr/bin/env python3
"""
Script to convert Masters Module Mermaid diagrams from markdown file to JPEG images
"""
    ...
    for i, line in enumerate(lines):
        if line.startswith('## '):
            ...
            if title_match:
                ...
                current_title = f"{num}. {title_text}"
        if '```mermaid' in line:
...
def main():
    ...
    for i, (diagram_code, title) in enumerate(zip(diagrams, titles), 1):
        # Clean title for filename - extract the main title part
        # Format: "1. User Authentication and Login Flow" -> "01_1-User-Authentication-and-Login-Flow"
        ...
        if title_match:
            ...
            filename = f"{int(num):02d}_{num}-{safe_title}.jpg"
        else:
            # Fallback if title doesn't match expected format

...
public class ActivityEnquiryReportController {
	...
	@PostMapping("/searchActivityEnquiryReport")
	public ResponseEntity<?> searchActivityEnquiryReport(@RequestBody SalesReportDto claimReport) {
		ApiResponse<List<ActivityEnquiryReportResponse>> response = new ApiResponse<>();
		Long count = 0l;
		List<ActivityEnquiryReportResponse> qaReportResponse = 
			salersReportRepo.activityEnquiryReport(userAuthentication.getManagementAccess(), 
	...
	@PostMapping("/downloadActivityEnquiryReport")
    public ResponseEntity<InputStreamResource> downloadActivityEnquiryReport(@RequestBody SalesReportDto claimReport, HttpServletResponse response) throws IOException{
    	
		List<ActivityEnquiryReportResponse> reportResponse = 
				salersReportRepo.activityEnquiryReport(userAuthentication.getManagementAccess(), 
						claimReport.getDealerId(), 
						claimReport.getBranchId(),
						claimReport.getProduct(), 
    	...
    	ByteArrayInputStream in = ExcelCellGenerator.acivityEnquiryReport(reportResponse);

        response.setContentType("application/vnd.ms-excel");

        HttpHeaders headers = new HttpHeaders();
        String filename = "Activity_Enquiry_Report_"+(Calendar.getInstance()).getTimeInMillis()+".xlsx";

#!/usr/bin/env python3
"""
Script to regenerate diagrams 6 and 7 from STORAGE_MODULE_SEQUENCE_DIAGRAMS.md
with fixed syntax
"""
...
def mermaid_to_jpeg(mermaid_code, output_path, title="Diagram", width=7680, height=4320):
    ...
    # Replace periods in method calls with spaces (e.g., e.printStackTrace -> e printStackTrace)
    ...
    # Remove all parentheses from message labels (they can cause parsing issues)
    # But preserve parentheses in method calls within the label text
    # Pattern: message: text (with parentheses) -> message: text without parentheses
    lines = cleaned_code.split('\n')
    cleaned_lines = []
    for line in lines:
    ...
    # Note over participants: text
    cleaned_code = re.sub(r'Note over ([^:]+):\s*([^\n]+)', r'Note over \1: \2', cleaned_code)
    
    # Remove curly braces from paths only (in GET requests)
    cleaned_code = re.sub(r'GET /api/files/\{filename\}', r'GET /api/files/filename', cleaned_code)
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if 'Syntax error' in page_content or 'syntax error' in page_content.lower():
                # Get the SVG inner HTML to check for error indicators
                svg_html = svg_element.inner_html()
                if 'Syntax error' in svg_html or 'error' in svg_html.lower()[:500]:
...
def main():
    """Main function to regenerate diagrams 6 and 7"""

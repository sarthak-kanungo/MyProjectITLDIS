...
export class AppComponent implements TokenResponseListener, IFrameRouteListener {
  ...
  ngOnInit() {
    ...
    this.router.events.pipe(filter(eve => eve instanceof NavigationEnd)).subscribe((fullUrl: NavigationEnd) => {
      const url = `training${fullUrl.urlAfterRedirects.split('?')[0]}`;
      this.iframeService.sendRouteChangeRequest(IFrameMessageSource.TRAINING, { url, queryParam: this.queryParam , firstChange: this.isRouteFirstChange} as UrlSegment);
      
      if (fullUrl.urlAfterRedirects.split('?')[0] === '/' || this.isRouteFirstChange) {
        this.isRouteFirstChange = false;
        return;
      }
      // const url = `warranty${fullUrl.urlAfterRedirects.split('?')[0]}`;
      // this.iframeService.sendRouteChangeRequest(IFrameMessageSource.WARRANTY, {url, queryParam:this.queryParam});
      });
    this.iframeService.sendTokenRequest(IFrameMessageSource.TRAINING);
  }

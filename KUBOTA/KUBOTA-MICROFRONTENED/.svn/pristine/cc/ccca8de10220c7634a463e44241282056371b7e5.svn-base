...
@Component({
  selector: 'app-closing-stock-report',
  templateUrl: './closing-stock-report.component.html',
  styleUrls: ['./closing-stock-report.component.css'],
  providers:[ReportService, TableDataService]
})
export class ClosingStockReportComponent implements OnInit {

  stockReportForm:FormGroup;
  isKai:boolean=true;
  jnewdate= new Date();
  key='ClosingStockReport';
  ...
  ngOnInit() {
    ...
    if (this.searchFilterValues != null || this.searchFilterValues != undefined && this.stockReportForm != null) {
      this.stockReportForm.patchValue(this.searchFilterValues)
      if(this.stockReportForm.get('stockOnDate').value){

      }else{
        this.stockReportForm.get('stockOnDate').patchValue(new Date());
      }
      this.searchData();
    }
    else{
    ...
    this.stockReportForm.controls.dealerCode.valueChanges.subscribe((res) => {
      if (res) {
        this.reportService.getDealerCodeList(res).subscribe(response => {
          ...
          if(typeof res === 'object'){
           ...
           this.stockReportForm.controls.branch.reset();
          }
        })
      }else{
        // this.getBranches(0);
  ...
  getBranches(dealerId){
    this.reportService.getBranchCodeList(dealerId).subscribe(res => {
      ...
      if(this.branches && this.branches.length==1){
        this.stockReportForm.controls.branch.patchValue(this.branches[0]['BRANCH_ID']);
      }
    });
  }
  ...
  statesDropdown(dealerId) {
    this.reportService.getStates(dealerId).subscribe(response => {
      ...
      this.stockReportForm.controls.state.reset();
    });
  }
  ...
  searchData(){
    let searchobj = this.stockReportForm.value;
    ...
    if (Object.keys(searchobj).length>0) {
      ...
      this.reportService.getStockOnDateReport(searchobj).subscribe(res => {
        if(!this.isKai){
  ...
  generateReport(){
    ...
    if (Object.keys(searchobj).length>0) {
      ...
      searchobj.branchId = searchobj.branch;
      this.reportService.exportStockOnDateReport(searchobj).subscribe((resp: HttpResponse<Blob>) => {
        if (resp) {
  ...
  getLevelByDeprtment() {
    if(this.controlsArr.length>0){
      this.controlsArr.forEach(controlName => {
        this.stockReportForm.removeControl(controlName);
      });
    }
    this.parentArray=[];
    ...
    this.reportService.getLevelByDepartment("SP").subscribe(res => {
      ...
      if(this.orgHierLevelList && this.orgHierLevelList.length>0){
        this.orgHierLevelList.forEach(obj => {
          ...
          this.stockReportForm.addControl(obj.LEVEL_CODE, new FormControl(obj.LEVEL_CODE));
  ...
  selectLevels(salesHoId, index) {
    ...
    this.stockReportForm.controls.orgHierarchyId.patchValue(this.orgHierarchyId);
  ...
  getDealerName(event){
    if (typeof event.option.value === 'object') {
      this.stockReportForm.controls.dealerName.patchValue(event.option.value.name);
    } else {
      this.stockReportForm.controls.dealerName.patchValue('');
    }
  }

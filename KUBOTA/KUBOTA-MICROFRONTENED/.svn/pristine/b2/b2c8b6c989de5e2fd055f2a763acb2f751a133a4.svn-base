...
@Component({
  selector: 'app-back-order-part-report',
  templateUrl: './back-order-part-report.component.html',
  styleUrls: ['./back-order-part-report.component.css'],
  providers: [ReportService]
})
export class BackOrderPartReportComponent implements OnInit {
  today = new Date();
  minDate: Date;
  maxDate: Date
  backOrderReportForm:FormGroup;
  isKai:boolean=true;
  jnewdate= new Date();
  key='BackOrderPartsReport';
  pageLoad:number=0
  ...
  ngOnInit() {
    this.backOrderReportForm = this.fb.group({
      fromDate:[null],
      toDate:[null],
      dealerCode:[null],
      branch:[null],
    ...
    if (this.searchFilterValues != null || this.searchFilterValues != undefined && this.backOrderReportForm != null) {
      this.backOrderReportForm.patchValue(this.searchFilterValues)
      
      this.searchData();
    }
    else{
      if(this.backOrderReportForm.get('fromDate').value){

      }else{
        let maxDate = new Date();
        maxDate.setMonth(maxDate.getMonth() - 1);
        this.backOrderReportForm.get('fromDate').patchValue(maxDate);
      }
      if(this.backOrderReportForm.get('toDate').value){

      }else{
        let maxDate = new Date();
        this.backOrderReportForm.get('toDate').patchValue(maxDate);
      }
      this.searchData();
    }

    this.backOrderReportForm.controls.dealerCode.valueChanges.subscribe((res) => {
      if (res) {
        this.reportService.getDealerCodeList(res).subscribe(response => {
          ...
          if(typeof res === 'object'){
           ...
           this.backOrderReportForm.controls.branch.reset();
          }
        })
      }else{
        // this.getBranches(0);
  ...
  getBranches(dealerId){
    this.reportService.getBranchCodeList(dealerId).subscribe(res => {
      ...
      if(this.branches && this.branches.length==1){
        this.backOrderReportForm.controls.branch.patchValue(this.branches[0]['BRANCH_ID']);
      }
    });
  }
  ...
  clearForm() {
    this.backOrderReportForm.reset();
    this.dataTable = null
  ...
  statesDropdown(dealerId) {
    this.reportService.getStates(dealerId).subscribe(response => {
      ...
      this.backOrderReportForm.controls.state.reset();
    });
  }
  ...
  searchData(){
    let searchobj = this.backOrderReportForm.value;
    ...
    if (Object.keys(searchobj).length>0) {
      ...
      this.reportService.getBackOrderPartsReport(searchobj).subscribe(res => {
        if(!this.isKai){
  ...
  generateReport(){
    ...
    if (Object.keys(searchobj).length>0) {
      ...
      searchobj.branchId = searchobj.branch;
      this.reportService.exportBackOrderPartsReport(searchobj).subscribe((resp: HttpResponse<Blob>) => {
        if (resp) {
  ...
  getLevelByDeprtment() {
    if(this.controlsArr.length>0){
      this.controlsArr.forEach(controlName => {
        this.backOrderReportForm.removeControl(controlName);
      });
    }
    this.parentArray=[];
    ...
    this.reportService.getLevelByDepartment("SP").subscribe(res => {
      ...
      if(this.orgHierLevelList && this.orgHierLevelList.length>0){
        this.orgHierLevelList.forEach(obj => {
          ...
          this.backOrderReportForm.addControl(obj.LEVEL_CODE, new FormControl(obj.LEVEL_CODE));
  ...
  selectLevels(salesHoId, index) {
    ...
    this.backOrderReportForm.controls.orgHierarchyId.patchValue(this.orgHierarchyId);
  ...
  getDealerName(event){
    if (typeof event.option.value === 'object') {
      this.backOrderReportForm.controls.dealerName.patchValue(event.option.value.name);
    } else {
      this.backOrderReportForm.controls.dealerName.patchValue('');
    }
  }
  ...
  public fromDateChange(event: object) {
    if (event && event['value']) {
      ...
      if (this.backOrderReportForm.get('toDate').value > this.maxDate)
        this.backOrderReportForm.get('toDate').patchValue(this.maxDate);
    }
  }
}

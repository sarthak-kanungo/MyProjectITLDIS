...
export class backOrderStore {
    ...
    private createBackOrderForm() {
        const backOrderForm = this.fb.group({
            dealerMaster:[{value:null,disabled:false}, Validators.compose([Validators.required,CustomValidators.selectFromLists()])],
    ...
    createItemDetailTableRow(data?) {
        ...
        let fg:FormGroup = this.fb.group({
            ...
            kaiAcceptedQty: [{ value: data?data.kaiAcceptedQty:null, disabled: true }],
             id: [{ value: data == undefined ? null : data.id, disabled: false }],
             cancelQty: [{value: data?data.cancelQty:null, disabled: false},Validators.required],

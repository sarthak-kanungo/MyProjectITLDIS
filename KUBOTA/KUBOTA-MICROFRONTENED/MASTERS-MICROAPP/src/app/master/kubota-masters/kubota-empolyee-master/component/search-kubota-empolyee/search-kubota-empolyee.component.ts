...
import { autoEmployeeCode } from './search-kubota-employee';
import { autoEmployeeName } from './search-kubota-employee';
import { SearchEmpMaster } from './search-kubota-employee';
...
@Component({
  selector: 'app-search-kubota-empolyee',
  templateUrl: './search-kubota-empolyee.component.html',
  styleUrls: ['./search-kubota-empolyee.component.scss']
})
export class SearchKubotaEmpolyeeComponent implements OnInit, AfterViewInit {
  ...
  autoEmployeeName(value) {
    this.searchkubotaEmployeeService.searchEmployeeName(value).subscribe(response => {
      console.log(response);
  ...
  //   this.searchkubotaEmployeeService.searchEmpMaster(searchValue).subscribe(searchRes => {
  ...
  searchEmployeeMaster() {
    ...
    this.searchkubotaEmployeeService.searchEmpMaster(searchFormValues).subscribe(res => {
  ...
  private changeActiveStatus(emitedId: ActionOnTableRecord) {
    this.searchkubotaEmployeeService.changeActiveStatus(emitedId.record['id']).subscribe(res => {
      console.log('res', res);

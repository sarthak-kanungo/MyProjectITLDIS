...
export class MaterialDetailsChargesService {
  ...
  getCsbNolist(value, model){
    return this.materialDetailApiService.getCsbNoList(value, model);
   }
  ...
  private getControlsConfigForTableFormArray(data = null) {
    return {
      ...
      csbNo: [{ value: data ? data.csbNo : null, disabled: false }],
      model:[{ value: data ? data.model : null, disabled: false }],
    }
  }
  getGSTPercentList() {

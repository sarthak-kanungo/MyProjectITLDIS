...
import { MarketingClaimReportService } from './marketing-claim-report.service';
import { saveAs } from 'file-saver';
import { BaseDto } from 'BaseDto';
import { activityStatusSearchDomain } from 'ActivityProposalModule';
...
import { SearchActivityClaimService } from '../../activity/activity-claim/component/search-activity-claim/search-activity-claim.service';
@Component({
  selector: 'app-marketing-claim-report',
  templateUrl: './marketing-claim-report.component.html',
  styleUrls: ['./marketing-claim-report.component.css'],
  providers: [SalesReportService, MarketingClaimReportService, MarketingActivitySearchContainerService, SearchActivityClaimService]
})
export class MarketingClaimReportComponent implements OnInit {
  ...
  pageTitle:string = 'Marketing Claim Report';

  activityStatusList:any[]
  activityNumberList:any[]
  dropdownActivityClaimStatus:any[]
  claimNumberList:any[]

  constructor(private reportService : SalesReportService,
    private claimService: MarketingClaimReportService,
  ...
  ngOnInit() {
    ...
    if(this.key=="activityReportStatus"){
      this.pageTitle = "Activity Report Status";
    }else if(this.key=="activityProposalStatus"){
      this.pageTitle = "Activity Proposal Status";
    }else if(this.key=="activityClaimStatus"){
      this.pageTitle = "Activity Claim Status";
    }
    ...
    this.searchActivityClaimService.getClaimStatus().subscribe(res => {
      this.dropdownActivityClaimStatus = res['result'];
    })
    ...
    this.searchGroupForm.controls.activityClaimNumber.valueChanges.subscribe(val => {
      if(val && typeof val == 'string'){        
        this.searchActivityClaimService.getClaimNumber(val).subscribe(response => {
          this.claimNumberList = response['result'];
        })
      }    
    })
  ...
  searchData(){
    ...
    if (Object.keys(searchobj).length>0) {
      ...
      this.claimService.getSearchRecords(searchobj, this.key).subscribe(res => {
  ...
  public removeNullFromObjectAndFormatDate(searchObject: object) {
    if (searchObject) {
      Object.keys(searchObject).forEach(element => {
        ...
        if (searchObject[element] && element === 'activityDate') {
          searchObject[element] = this.dateService.getDateIntoYYYYMMDD(searchObject[element])
        }
        if (searchObject[element] && element === 'activityClaimNumber') {
          searchObject[element] = searchObject[element]['claimNumber']
        }
  ...
  displayClaimNumberFn(selectedOption){
    if (!!selectedOption && typeof selectedOption === 'string') {
      return selectedOption;
    }
    return selectedOption ? selectedOption['claimNumber'] : undefined;
  }

  marketingReport(event){
  ...
  downloadExcel(data){
    this.claimService.downloadReport(data, this.key).subscribe((resp: HttpResponse<Blob>) => {
      if (resp) {

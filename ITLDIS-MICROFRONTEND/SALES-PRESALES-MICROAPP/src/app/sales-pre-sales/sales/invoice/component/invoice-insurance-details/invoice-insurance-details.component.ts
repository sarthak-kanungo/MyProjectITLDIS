...
export class InvoiceInsuranceDetailsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.insuranceForm.get('policyStartDate').valueChanges.subscribe((dateRes: Date) => {
      console.log('dateRes', dateRes);
      if (dateRes && !this.isCancelInvoice && !this.isView) {
        let policyStartDate = this.dateService.getDateIntoYYYYMMDD(dateRes);
        const dateArray = policyStartDate.split('-');
        this.minPolicyExpiryDate = new Date(`${ dateRes.getFullYear() + 1 }-${ dateArray[1] }-${ dateArray[2] }`);
        // this.insuranceForm.get('policyExpiryDate').patchValue(this.minPolicyExpiryDate);
      }
    });
  }
  ngOnChanges(changes: import("@angular/core").SimpleChanges): void {
    ...
    if (changes.isCancelInvoice || changes.isView) {
      this.disableInsuranceFormAtCancelInvoice();
    }
  }
  private disableInsuranceFormAtCancelInvoice() {
    if (this.insuranceForm && (this.isCancelInvoice || this.isView)) {
  ...
  private patchValueToInsuranceForm() {
    if (this.insuranceForm && this.invoiceInsuranceDetail) {
      this.insuranceForm.get('insuranceCompanyMaster').patchValue({
        ...
        id:this.invoiceInsuranceDetail.id
      });

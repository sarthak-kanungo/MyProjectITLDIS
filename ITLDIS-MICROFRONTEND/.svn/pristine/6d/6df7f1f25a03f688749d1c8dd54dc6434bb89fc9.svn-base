...
export class JobCardPresenter {
	...
	// add for retrofitment
    //  get retrofitment(){
	// 	return this.serviceFormGroup.get('retrofitment')as FormGroup
	//  }
	...
	addRowFrsView(list: LabourChargesItem) {
		...
		addedNewLabourCharges.get('category').disable()
	...
	addRowJobCode(list: JobCodeTableData) {
		...
		if(jobCodeDetails.valid){
		    ...
		    if (this.isGoowillShow && list && (list.category['category']=='Warranty' || list.category['category']=='FOC')) {
                addedNewJobCode.get('amount').disable();
            }
            jobCodeDetails.push(addedNewJobCode);
            return addedNewJobCode;
		}
		return null;
	}
	...
	addRowFrs(list: LabourChargesItem) {
		...
		if(labourDetails.valid || labourDetails.length===0){
	...
	addRow(list: SparePartRequisitionItem) {
		...
		if(machineDetails.length==0 || machineDetails.valid){
		    let fg:FormGroup = this.createMachineDetailsTableRow(list);
		    machineDetails.push(fg);
		    return fg;
		}
		return null;
	}
	addRowForDisable(list: SparePartRequisitionItem, dataForCheckCondition: ViewJobCard) {
		...
		if (currentResp.get('partNumber').value) {
			...
			if (this.isGoowillShow && list && (list.category['category']=='Warranty' || list.category['category']=='FOC')) {
			    currentResp.get('quantity').disable();
			    currentResp.get('qtyChangeFlag').patchValue(false);
			}
		}
	...
	patchDataFromView(viewData: ViewJobCard) {
		...
		if (viewData.serviceRepresentative) {
			viewData.serviceRepresentative.name = viewData.serviceRepresentative.name.trim();
		...
		this.jobCardBasicInfo.get('closeRemark').patchValue(viewData.closeRemark);
		this.jobCardBasicInfo.get('closeDelayReason').patchValue(viewData.closeDelayReason);
		...
		if (viewData.mechanic) {
			viewData.mechanic.name = viewData.mechanic.name.trim();
			this.jobServiceCard.get('mechanicName').patchValue(viewData.mechanic.name)
		}
		this.jobCardBasicInfo.get('id').patchValue(viewData.id);
		...
		if (viewData.closedDate != null) {
			const closedDate = new Date(viewData.closedDate)
			this.jobCardBasicInfo.get('closedDate').patchValue(this.dateService.getDateIntoDDMMYYYY(closedDate))
		}
		...
		this.jobCardBasicInfo.get('status').patchValue(viewData.status)
		...
		this.jobCardBasicInfo.get('warrantyEndDate').patchValue(this.dateService.stringToDateIntoDDMMYYYY(viewData.warrantyEndDate))
		this.jobCardBasicInfo.get('totalWarrantyHour').patchValue(viewData.totalWarrantyHour)
		...
		// if (viewData.serviceType.serviceType == "Retro Fitment") {
		// 	this.jobCardBasicInfo.get('retroFitment').patchValue(true)
		// }
		...
		this.jobCardBasicInfo.patchValue({
			serviceCategory: viewData.serviceCategory.category,
			...
			timein: viewData.taskTime,
			 machineOutDate : viewData.outDateTime?(viewData.outDateTime.split(' '))[0]:null,
			 timeout : viewData.outDateTime?(viewData.outDateTime.split(' '))[1]:null,
			...
			meterChangeHour:viewData.meterChangeHour,
		...
		if (viewData.labourCharge.length > 0) {
			this.patchToLabourCahrges(viewData as ViewJobCard)
		}
		if (viewData.outsideJobCharge.length > 0) {
			this.patchToOusideLabourJobCard(viewData as ViewJobCard)
		}
		...
		viewData.serviceJobcardPhotos.forEach(photos => {
			...
			// if (photos.fileType == 'Signed Job Card') {
			// 	this.jobPdf.push(photos.fileName)
			// }
			// if (photos.fileType == 'Retrofitment Acknowledgement Form') {
			// 	this.ackPdf.push(photos.fileName)
			// }
		})
	}
	...
	private patchToLabourCahrges(jobcardData: ViewJobCard) {
		
		(this.labourCharges.get('labourChargesControl') as FormArray).clear();
		const labourChargesData = []
		jobcardData.labourCharge.forEach(data => {
		...
		labourChargesData.forEach(ele => {
			if (this.operation === Operation.EDIT && jobcardData.status==='Open') {
				// console.log('forrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr laaabourrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr chargeeeeeeeeeeeeeeeeeeeee',ele,jobcardData.status,Operation.EDIT,this.operation)
				this.addRowFrs(ele)
			} else {
	...
	private patchToOusideLabourJobCard(jobcardData: ViewJobCard) {
		(this.jobCode.get('jobCodeControl') as FormArray).clear();
		const jobodeDataStore = []
		jobcardData.outsideJobCharge.forEach(data => {
	...
	private patchToPartRequisition(
		...
		(this.partRequisition.get('partRequisitionControl') as FormArray).clear();
		...
		partRequsitionData.forEach(ele => {
			if (this.operation === Operation.EDIT && partRequisitionItemsForPatch.status==='Open') {
	...
	// debuge for patch chassis number
	patchValueFromChassisNumber(selectedChassisNumber: ChassisNumberJobData) {
		...
		/*if (selectedChassisNumber.dealerName) {
			this.jobCardBasicInfo.get('solddealer').patchValue(selectedChassisNumber.dealerName)
		}*/
		// if(selectedChassisNumber.){
		// 	this.jobCardBasicInfo.get('retroFitment').patchValue(selectedChassisNumber.flag)
		// }
		...
		if (selectedChassisNumber.totalWarrantyHour) {
			this.jobCardBasicInfo.get('totalWarrantyHour').patchValue(selectedChassisNumber.totalWarrantyHour)
		}
		if (selectedChassisNumber.warrantyEndDate) {
			this.jobCardBasicInfo.get('warrantyEndDate').patchValue(this.dateService.stringToDateIntoDDMMYYYY(selectedChassisNumber.warrantyEndDate))
		}
	...
	// Added By Ankit 26-04-2023 Ankit
	jobCardfile(files) {
        this.jobPdf = files
    }

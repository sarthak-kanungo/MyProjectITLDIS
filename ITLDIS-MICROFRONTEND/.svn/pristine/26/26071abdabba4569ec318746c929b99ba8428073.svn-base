...
@Component({
  ...
  styleUrls: [
    './ngsw-search-table.component.scss',
    './fixed-table-header.scss'],
...
export class NgswSearchTableComponent implements OnInit, AfterViewInit, OnChanges, AfterContentInit {
  ...
  // for checkbox
   ...
   @Output() 
   public multiRecordSelect = new EventEmitter<object>();
   @Input ()  forcheckBox:boolean=false
   @Input() showMultiDeleteCheckbox:boolean=false;
   @Input ()  isShowMultiDeleteButton:boolean=false
  // 
  ...
  @Output()
  public blockData = new EventEmitter();
  @Output()
  public actionOnTableRecord = new EventEmitter<object>();
  @Output()
  public actionOnHeaderRecord = new EventEmitter<object>();
  @Output()
  ...
  private initDataTable(): DataTable {
    return { headerRow: [], tableBody: {}, titleMap : {} } as DataTable;
  }
  ...
  public emitPageChange(event: { pageIndex: number; pageSize: number;previousPageIndex: number; }) {
  ...
  emitMultiSelectData(){
     this.multiRecordSelect.emit();
  }
  checkAll(event:any,message:string){
    if(event.checked){
      // console.log('deep after select',this.dataTableCopy)
      // console.log('deep after select',this.dataTableCopy.tableBody)
      // console.log('deep after select with message',this.dataTableCopy.tableBody,message)
      this.actionOnHeaderRecord.emit(this.dataTableCopy.tableBody);
      
      if(this.dataTableCopy.tableBody.content.length>0){
  ...
  if(!event.checked){
   ...
   this.forcheckBox=!this.forcheckBox
  }
 
// public emitActionOnHeaderRecord(record: object, btnAction: string, recordIndex: number
//   ) {
//     // console.log('emited currency edit record', record);
//     if (record) {
//      const data = {

...
export class InvoiceAccessoryDetailService {
  ...
  createTotalInvoiceCalcForm() {
    this.totalInvoiceCalcForm = this.fb.group({
      ...
      rtoCharges: [null, Validators.compose([Validators.required, CustomValidators.floatNumber])],
      insuranceCharges: [null, [Validators.required, CustomValidators.floatNumber]],
  ...
  private setCustomErrorToRate(fg: FormGroup) {
    if (fg.get('discount').value!=null && fg.get('discount').value!='' && this.totalValue < fg.get('discount').value) {
  ...
  private setDiscountCustomValidators(fg: FormGroup) {
    fg.get('discount').setValidators(ValidateMax(this.totalValue, 'value should not exceed to rate'));

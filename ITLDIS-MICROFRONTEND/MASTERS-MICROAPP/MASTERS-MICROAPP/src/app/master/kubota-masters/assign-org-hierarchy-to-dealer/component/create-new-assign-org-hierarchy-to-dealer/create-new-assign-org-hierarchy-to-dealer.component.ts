...
export class CreateNewAssignOrgHierarchyToDealerComponent implements OnInit {
  ...
  private buildJsonForSaveitldisDealerFeildHier() {
    const saveitldisDealerFeildHier = {
      ...this.buildJsonForCreateitldisDealerFeildHier()
    }
    return saveitldisDealerFeildHier;
  }

  buildJsonForCreateitldisDealerFeildHier() {
    ...
    for (let i = 0; i < this.tableShowListAnother.length; i++) {
      ...
      for (let j = 0; j < tableDataSubmit.length; j++) {
        if (tableDataSubmit[j].orgHierarchyId == tableDataSubmit[j].lastOrgHierarchyId) {
          let SubmititldisDealerFeildHier = {} as SubmitAssignOrgHierarchyToDealerDto
          SubmititldisDealerFeildHier.orgHierarchyId = tableDataSubmit[j].orgHierarchyId ? tableDataSubmit[j].orgHierarchyId : null
          ...
          SubmititldisDealerFeildHier.dealerHoDepartmentId = DealerHoDepartment;
          this.submitFinalList.push(SubmititldisDealerFeildHier)
          break;
        }
      }
    }
    console.log(this.submitFinalList)
    console.log("...SubmititldisDealerFeildHier..")
    return this.submitFinalList;
  }
  submitData() {
    this.createNewAssignOrgHierarchyToDealerService.submitAssignOrgHierarchyToDealer(this.buildJsonForCreateitldisDealerFeildHier()).subscribe(res => {
      if (res) {
  ...
  private openConfirmDialog(): void | any {
    let message = 'Do you want to submit itldis Dealer Fileld Hierarchy?';
    if (this.isEdit) {
      message = 'Do you want to update itldis Dealer Fileld Hierarchy?';
    }
  ...
  private openConfirmDialogForDeleteRow(indexNumber: any): void | any {
    let message = 'Do you want to delete itldis Dealer Fileld Hierarchy?';
    const confirmationData = this.setConfirmationModalData(message);
    console.log('confirmationData', confirmationData);
    const dialogRef = this.dialog.open(ConfirmationBoxComponent, {
      width: '500px',

...
import { SearchitldisEmployeeService } from '../search-itldis-empolyee/searchitldis-employee.service';
...
export class EmpolyeeDetailsComponent implements OnInit {
 ...
 // kaibranchList: BaseDto<Array<DropDoenKaibranch>>
  ...
  checkEmployeeCode(){    
    if (this.isCreate) {
      this.employeeDetailsForm.controls.employeeCode.valueChanges.subscribe(empCode => {
        this.searchitldisEmployeeService.searchEmployeeCode(empCode).subscribe(response => {      
  ...
  private openConfirmDialog(): void | any {
    let message = 'Do you want to submit itldis Employee Master?';
    if (this.isEdit) {
       message = 'Do you want to update itldis Employee Master?';
    } 
  ...
  pathchData(empId){
    let index = 0;
    this.emplyeeDetailsService.getitldisEmployeeById(empId).subscribe(emp => {

...
export class ItemMovementReportComponent implements OnInit {
  ...
  minDate: Date;
  maxDate: Date
  itemMovementReportForm:FormGroup;
  isKai:boolean=true;
  jnewdate= new Date();
  key='ItemMovementReport';
  ...
  ngOnInit() {
    this.itemMovementReportForm = this.fb.group({
      fromDate:[null],
      toDate:[null],
      dealerCode:[null],
      branch:[null],
    ...
    if (this.searchFilterValues != null || this.searchFilterValues != undefined && this.itemMovementReportForm != null) {
      this.itemMovementReportForm.patchValue(this.searchFilterValues)
      
      this.searchData();
    }
    else{
      if(this.itemMovementReportForm.get('fromDate').value){

      }else{
        let maxDate = new Date();
        maxDate.setMonth(maxDate.getMonth() - 1);
        this.itemMovementReportForm.get('fromDate').patchValue(maxDate);
      }
      if(this.itemMovementReportForm.get('toDate').value){

      }else{
        let maxDate = new Date();
        this.itemMovementReportForm.get('toDate').patchValue(maxDate);
      }
      this.searchData();
    }

    this.itemMovementReportForm.controls.dealerCode.valueChanges.subscribe((res) => {
      if (res) {
        this.reportService.getDealerCodeList(res).subscribe(response => {
          ...
          if(typeof res === 'object'){
           ...
           this.itemMovementReportForm.controls.branch.reset();
          }
        })
      }else{
        // this.getBranches(0);
    ...
    this.itemMovementReportForm.get('partNumber').valueChanges.subscribe((changedValue: string) => {
      if (changedValue) {
  ...
  getBranches(dealerId){
    this.reportService.getBranchCodeList(dealerId).subscribe(res => {
      ...
      if(this.branches && this.branches.length==1){
        this.itemMovementReportForm.controls.branch.patchValue(this.branches[0]['BRANCH_ID']);
      }
    });
  }
  ...
  clearForm() {
    this.itemMovementReportForm.reset();
    this.dataTable = null
  ...
  statesDropdown(dealerId) {
    this.reportService.getStates(dealerId).subscribe(response => {
      ...
      this.itemMovementReportForm.controls.state.reset();
    });
  }
  ...
  searchData(){
    let searchobj = this.itemMovementReportForm.value;
    ...
    if (Object.keys(searchobj).length>0) {
      ...
      this.reportService.getItemMovementReport(searchobj).subscribe(res => {
        if(!this.isKai){
  ...
  generateReport(){
    ...
    if (Object.keys(searchobj).length>0) {
      ...
      searchobj.branchId = searchobj.branch;
      this.reportService.exportItemMovementReport(searchobj).subscribe((resp: HttpResponse<Blob>) => {
        if (resp) {
  ...
  getLevelByDeprtment() {
    if(this.controlsArr.length>0){
      this.controlsArr.forEach(controlName => {
        this.itemMovementReportForm.removeControl(controlName);
      });
    }
    this.parentArray=[];
    ...
    this.reportService.getLevelByDepartment("SP").subscribe(res => {
      ...
      if(this.orgHierLevelList && this.orgHierLevelList.length>0){
        this.orgHierLevelList.forEach(obj => {
          ...
          this.itemMovementReportForm.addControl(obj.LEVEL_CODE, new FormControl(obj.LEVEL_CODE));
  ...
  selectLevels(salesHoId, index) {
    ...
    this.itemMovementReportForm.controls.orgHierarchyId.patchValue(this.orgHierarchyId);
  ...
  getDealerName(event){
    if (typeof event.option.value === 'object') {
      this.itemMovementReportForm.controls.dealerName.patchValue(event.option.value.name);
    } else {
      this.itemMovementReportForm.controls.dealerName.patchValue('');
    }
  }
  ...
  public fromDateChange(event: object) {
    if (event && event['value']) {
      ...
      if (this.itemMovementReportForm.get('toDate').value > this.maxDate)
        this.itemMovementReportForm.get('toDate').patchValue(this.maxDate);
    }
  }
}

...
export class CustomValidators {
    ...
    static selectFromListDynamic(msg = 'Please select from list') {
        return (control: AbstractControl) => {
...
export function ValidateMin(min: AbstractControl, msg?: string) {
    return (control: AbstractControl) => {
        try {
            let numVal = parseFloat(`${ control.value }`);
            let minVal = parseFloat(`${ min.value }`);

            if (isNaN(minVal) || (typeof numVal === 'number' && typeof minVal === 'number' && !isNaN(numVal) && !isNaN(minVal) && numVal >= minVal)) {
                if(!isNaN(minVal))
                    control.patchValue(numVal, { onlySelf: true, emitEvent: false });
                return null
            }
            return { minValue: true, msg: msg ? msg : 'value should not less' };

...
@Component({
  selector: 'app-installation-monitoring-board',
  templateUrl: './installation-monitoring-board.component.html',
  styleUrls: ['./installation-monitoring-board.component.css'],
  providers: [InstallationMonitoringBoardService, ServiceMonitoringBoardService, MrcSearchWebService]
})
export class InstallationMonitoringBoardComponent implements OnInit {

  imbSearchForm:FormGroup
  ...
  constructor(private fb: FormBuilder,
    ...
    private imbService: InstallationMonitoringBoardService,
    private tableDataService: NgswSearchTableService,
    private dateService: DateService,
    private toastr: ToastrService) { 
      this.tomorrow.setDate(this.tomorrow.getDate() + 15);
    }

  ngOnInit() {
    this.imbSearchForm = this.fb.group({
      fromDate : [null],
      toDate: [null],
      product: [null],
      series: [null],
    ...
    if (this.searchFilterValues != null || this.searchFilterValues != undefined && this.imbSearchForm != null) {
      this.imbSearchForm.patchValue(this.searchFilterValues)
      // this.searchData();
    }
    else{
      ...
      this.imbSearchForm.get('fromDate').patchValue(backDate);
      this.imbSearchForm.get('toDate').patchValue(new Date());
      // this.searchData();
    }
    this.getBranches(0);
    this.getChassis()
    ...
    this.imbSearchForm.controls.dealerCode.valueChanges.subscribe((res) => {
      if (res) {
        this.smbService.getDealerCodeList(res).subscribe(response => {
          ...
          if(typeof res === 'object'){
           ...
           this.imbSearchForm.controls.branch.reset();
          }
        })
      }else{
        // this.getBranches(0);
  ...
  getBranches(dealerId){
    this.smbService.getBranchCodeList(dealerId).subscribe(res => {
      ...
      if(this.branches && this.branches.length==1){
        this.imbSearchForm.controls.branch.patchValue(this.branches[0]['BRANCH_ID']);
      }
    });
  }
  ...
  private getChassis(){
  this.imbSearchForm.get('chassisNo').valueChanges.subscribe( txt => {
    if(txt){
  ...
  setToDate(event: MatDatepickerInput<Date>) {
    if (event && event['value']) {
      ...
      if (this.imbSearchForm.get('toDate').value > this.maxDate)
        this.imbSearchForm.get('toDate').patchValue(this.maxDate);
    }
  }
  selectProduct(value) {
  ...
  getLevelByDeprtment() {
    if(this.controlsArr.length>0){
      this.controlsArr.forEach(controlName => {
        this.imbSearchForm.removeControl(controlName);
      });
    }
    this.parentArray=[];
    ...
    this.mrcSearchService.getLevelByDepartment("SE").subscribe(res => {
      ...
      if(this.orgHierLevelList && this.orgHierLevelList.length>0){
        this.orgHierLevelList.forEach(obj => {
          ...
          this.imbSearchForm.addControl(obj.LEVEL_CODE, new FormControl(obj.LEVEL_CODE));
  ...
  selectLevels(salesHoId, index) {
    ...
    this.imbSearchForm.controls.orgHierarchyId.patchValue(this.orgHierarchyId);
  ...
  getDealerName(event){
    if (typeof event.option.value === 'object') {
      this.imbSearchForm.controls.dealerName.patchValue(event.option.value.name);
    } else {
      this.imbSearchForm.controls.dealerName.patchValue('');
    }
  }
  ...
  searchData(){
    let searchobj = this.imbSearchForm.value;
    ...
    if (Object.keys(searchobj).length>0) {
      ...
      this.imbService.getIMBSearchRecords(searchobj).subscribe(res => {
  ...
  clickExportToExcel(){
    ...
    if (Object.keys(searchobj).length>0) {
      ...
      searchobj.branchId = searchobj.branch;
      this.imbService.exportIMBSearchRecords(searchobj).subscribe((resp: HttpResponse<Blob>) => {
        if (resp) {

...
import { BaseDto } from '../webservice-config/basedto';
...
export class UserAccessPermissionService {
  private readonly checkRouterAccessibilityUrl = `${ environment.baseUrl }${ urlService.api }${ urlService.itldisuser }${ urlService.checkRouterAccessibility }`;
  constructor(
    ...
    if (loginUser && loginUser.id) {
      return this.httpClient.get<BaseDto<boolean>>(this.checkRouterAccessibilityUrl, {
        params: new HttpParams()
          .set('userId', loginUser.loginUserId+"")
...
export class UserAccessGuard implements CanActivate {
  ...
  canActivate(
    ...
    return this.userAccessPermissionService.checkPermission(state.url).toPromise();
  }

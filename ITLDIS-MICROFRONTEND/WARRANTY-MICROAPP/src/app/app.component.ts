...
export class AppComponent implements TokenResponseListener, IFrameRouteListener {
  ...
  ngOnInit() {
    ...
    this.router.events.pipe(filter(eve => eve instanceof NavigationEnd)).subscribe((fullUrl: NavigationEnd) => {
      ...
      this.iframeService.sendRouteChangeRequest(IFrameMessageSource.WARRANTY, { url, queryParam: this.queryParam , firstChange: this.isRouteFirstChange} as UrlSegment);
    ...
    this.iframeService.sendTokenRequest(IFrameMessageSource.WARRANTY);
  }

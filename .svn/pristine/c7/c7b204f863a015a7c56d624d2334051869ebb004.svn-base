...
@Component({
  selector: 'app-non-moving-part-stock-report',
  templateUrl: './non-moving-part-stock-report.component.html',
  styleUrls: ['./non-moving-part-stock-report.component.css'],
  providers: [ReportService, TableDataService]
})
export class NonMovingPartStockReportComponent implements OnInit {

  stockReportForm:FormGroup;
  isKai:boolean=true;
  jnewdate= new Date();
  key='NonMovingPartsStockReport';
  ...
  searchData(){
    ...
    if (Object.keys(searchobj).length>0) {
      ...
      this.reportService.getNonMovingPartStockReport(searchobj).subscribe(res => {
        if(!this.isKai){
  ...
  generateReport(){
    ...
    if (Object.keys(searchobj).length>0) {
      ...
      searchobj.branchId = searchobj.branch;
      this.reportService.exportNonMovingPartStockReport(searchobj).subscribe((resp: HttpResponse<Blob>) => {
        if (resp) {

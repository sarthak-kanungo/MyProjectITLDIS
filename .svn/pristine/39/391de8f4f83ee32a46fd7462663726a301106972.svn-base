...
import { autoEmployeeCode, autoEmployeeName, SearchEmpMaster } from './search-kubota-employee';

@Injectable({
  providedIn: 'root'
})
export class SearchkubotaEmployeeService {
private readonly searchEmployeeCodeUrl=`${environment.baseUrl}${urlService.api}${urlService.kubotaemployee}${urlService.employeeCodeAuto}`
private readonly searchEmployeeNameUrl=`${environment.baseUrl}${urlService.api}${urlService.kubotaemployee}${urlService.employeeNameAuto}`
private readonly searchEmpMasterUrl=`${environment.baseUrl}${urlService.api}${urlService.kubotaemployee}${urlService.searchEmployee}`
private readonly changeActiveStatusUrl=`${environment.baseUrl}${urlService.api}${urlService.kubotaemployee}${urlService.changeActiveStatus}`

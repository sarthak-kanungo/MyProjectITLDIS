#!/usr/bin/env python3
"""
Script to convert ITLDIS Report Module UML Class Diagram from markdown file to JPEG image
"""
...
def mermaid_to_jpeg(mermaid_code, output_path, title="Report Module UML Diagram", width=16000, height=12000):
...
{cleaned_code}
    ...
    with sync_playwright() as p:
        ...
        # Playwright has limits, so use max 12000x9000
        max_viewport_width = 12000
        max_viewport_height = 9000
        final_width = min(max(width, int(svg_width * 0.7)), max_viewport_width)
        final_height = min(max(height, int(svg_height * 0.7)), max_viewport_height)
        ...
        # Try SVG element screenshot first (more reliable for large diagrams)
        svg_element = page.query_selector('svg')
        if svg_element:
            try:
                ...
                if bbox:
                    print(f"   SVG bounding box: {bbox['width']}x{bbox['height']}")
                    screenshot_bytes = svg_element.screenshot(type='png', timeout=120000)
                    print(f"   SVG element screenshot successful")
                else:
                    raise Exception("SVG bounding box not available")
            except Exception as e:
                error_msg = str(e)[:150]
                print(f"   SVG element screenshot failed: {error_msg}")
                ...
                try:
                    ...
                    print(f"   Full page screenshot failed: {error_msg2}")
                    # Last resort: try viewport screenshot
                    screenshot_bytes = page.screenshot(
                        type='png',
                        timeout=120000
                    )
                    print(f"   Viewport screenshot used as fallback")
        else:
            # No SVG element, try full page
            screenshot_bytes = page.screenshot(
                full_page=True,
                type='png',
                timeout=120000
            )
            print(f"   Full page screenshot successful (no SVG element)")
...
def main():
    """Main function to convert Report Module UML diagram"""
    ...
    markdown_file = script_dir / "REPORT-MODULE-UML-DIAGRAM.md"
    ...
    output_filename = "Report-Module-UML-Class-Diagram.png"
    ...
    try:
        ...
        old_jpg = output_dir / "Report-Module-UML-Class-Diagram.jpg"
        if old_jpg.exists():
            old_jpg.unlink()
            print(f"   Deleted old JPG file: {old_jpg}")
        
        mermaid_to_jpeg(diagram, output_path, "Report Module UML Class Diagram", width=16000, height=12000)

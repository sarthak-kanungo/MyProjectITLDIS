...
<script language="JavaScript" type="text/javascript">
    ...
    function cUDAction(str, row,strVal,workDesc)
    {
        if(str == 'add'){
            var elementArr=new Array('Work Description');
            var strValue=null;
            var strObject=null;
            ...
            var name = TrimAll(document.getElementById("Work Description").value).replace('&','@@');
            var url="<%=cntxpath%>/masterAction.do?option=manageOtherJobWork&type="+str+"&name="+name+"&t="+new Date().getTime();
        }
        else{
            var url="<%=cntxpath%>/masterAction.do?option=manageOtherJobWork&status="+str+"&workDesc="+workDesc+"&name="+name+"&type="+str+"&id="+strVal+"&t="+new Date().getTime();
        }
        //alert(url);
            ...
            if (xmlHttp.readyState == 4 || xmlHttp.readyState == "complete")
            {
                ...
                else
                {
                    ...
                    if(str != 'add' && res.split("@@")[0] =='Success'){
                        if(str=='Y'){
                            document.getElementById('st'+row).innerHTML="<a href=\"javascript:void(0)\" onclick=\"cUDAction('N','"+row+"','"+strVal+"','"+workDesc+"')\"  class=\"headingSpas\" style=\"color: blue;\" >N</a>";
                        }
                        else if(str=='N'){
                            document.getElementById('st'+row).innerHTML="<a href=\"javascript:void(0)\" onclick=\"cUDAction('Y','"+row+"','"+strVal+"','"+workDesc+"')\"  class=\"headingSpas\" style=\"color: blue;\" >Y</a>";
                        }
                    }
                    if(str=='Y' && res.split("@@")[0] =='workExist' ){
                        document.getElementById('st'+row).innerHTML="<a href=\"javascript:void(0)\" onclick=\"cUDAction('Y','"+row+"','"+strVal+"','"+workDesc+"')\"  class=\"headingSpas\" style=\"color: blue;\" >Y</a>";
                    }
                    if(str == 'add' && res.split("@@")[0] =='Success'){
                        //submitForm();
    ...
    function updateAction(str, row,id)
    {
        if(document.getElementById("newName"+row).value==""){
            ...
            document.getElementById("msg_saveFAILED").style.display = "";
            document.getElementById("msg_saveFAILED").innerHTML="Please Enter Value in Work Description Field";
            return false;
        }
        ...
        if(temp)
        {
            document.getElementById("newName"+row).focus();
            document.getElementById("msg_saveFAILED").style.display = "";
            document.getElementById("msg_saveFAILED").innerHTML="Special Characters are not allowed in Work Description Field";
            window.scrollTo(0,0);//go to top of page
            return false;
        }

        var name = TrimAll(document.getElementById("newName"+row).value).replace('&','@@');
        var url="<%=cntxpath%>/masterAction.do?option=manageOtherJobWork&id="+id+"&name="+name+"&type="+str+"&t="+new Date().getTime();
...
<div class="contentmain1">
    <div class="breadcrumb_container">
        <ul>
            <li class="breadcrumb_link"><a href='<%=cntxpath%>/masterAction.do?option=initOptions'>MASTERS</a>OTHER JOB WORK </li>
                ...
                <table cellspacing=1 cellpadding=5 border=0 width=100%>
                    ...
                    <tr height=30 bgcolor="#FFFFFF">
                        <td align= center class="headingSpas">
                            <html:form action="masterAction.do?option=initOtherJobWork&param=search" method="post" styleId="searchBy" onsubmit="donotsubmit();">
                                <table width=100%  border=0 cellspacing=0 cellpadding=0 bgcolor=#cccccc>
                                    <tr bgcolor="#FFFFFF">

                                        <td  style="padding-left:10px" width="20%" class="headingSpas" nowrap align="left">Work Description</td>
                        ...
                        <c:if test="${!empty otherJobWorkList}">
                        <tr height=25 bgcolor="#FFFFFF">
                            <td align= center class="headingSpas">
                                <pg:pager url="masterAction.do" maxIndexPages="10" maxPageItems="10">
                                    <pg:param name="option" value="initOtherJobWork"/>
                                    ...
                                    <table  id="data" width=100%  border=0 cellspacing=1 cellpadding=1 bordercolor=#cccccc bgcolor=#cccccc>
                                        <tr  bgcolor="#eeeeee" height="20">
                                            ...
                                            <td   align="left"  class="headingSpas" nowrap><b>Work Description</b></td>
                                        ...
                                        <logic:iterate id="otherJobWorkList" name="otherJobWorkList">
                                            <pg:item>
                                                <tr id ="${sno}" height="20">
                                                    <td align="center" bgcolor="#FFFFFF" width="35" class="headingSpas" style="padding-left:5px; padding-right: 5px">${sno}</td>
                                                    <td align="left" bgcolor="#FFFFFF" title="${otherJobWorkList.name}" class="headingSpas" style="padding-left:5px; padding-right: 5px">
                                                        <span id ="compType${sno}" >
                                                            ${otherJobWorkList.name}
                                                        </span>
                                                    </td>
                                                    ...
                                                    <td align="center" bgcolor="#FFFFFF" width="40" class="headingSpas" style="padding-left:5px; padding-right: 5px">
                                                        <span id ="imageButton${sno}" >
                                                            <img src="${pageContext.request.contextPath}/images/edit.gif" onclick="editRow('${otherJobWorkList.name}', ${sno},'${otherJobWorkList.id}')" title="Edit"/>
                                                        </span>
                                                    </td>
                                                    <td align="center" bgcolor="#FFFFFF" width="35" class="headingSpas" style="padding-left:5px; padding-right: 5px">
                                                        <span id ="st${sno}" >
                                                            <a href="javascript:void(0)" onclick="cUDAction('${otherJobWorkList.status}', ${sno},'${otherJobWorkList.id}','${otherJobWorkList.name}')"  class="headingSpas" style="color: blue;" >
                                                                ${otherJobWorkList.status}&nbsp;
                                                            </a>
                                                        </span>
                                                    </td>
                                                </tr>
                        ...
                        <c:if test="${empty otherJobWorkList}">
                            <tr bgcolor="#FFFFFF">
                                <td valign="top" class="headingSpas" style="padding-top:10px;color: red">
                                    ...
                                    Work Description '<%=request.getParameter("nameSrch") == null ? "" : request.getParameter("nameSrch")%>' not found
                                    <%}%>
                                </td>
                            </tr>
                        </c:if>
                                            ...
                                            <table width="100%" border="0" cellpadding="0" cellspacing="0" >
                                                <tr height="30" bgcolor="#FFFFFF">
                                                    ...
                                                    <td style="padding-left:10px;padding-top:5px " width="30%">
                                                        <input type="text" name="name" size='200'  id="Work Description" class="headingSpas" onkeypress="return CommentsMaxLength(this, 200);" maxlength="200" style="width:170px" onblur="this.value=TrimAll(this.value)"/>
                                                    </td>

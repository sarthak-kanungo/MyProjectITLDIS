 ...
 */
public class EAMG_SAP_PartAction extends org.apache.struts.action.Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
        ...
        try {
            ...
            xls_validate_result = new EAMG_ValidateXlsWithTemplate().isSAPPartExcelValidated(xlsfile, conn);

            //if validation successful.
            if (xls_validate_result.equals("success")) {
                insertpart = new EAMG_InsertPartParameters();
                //insertion of PARTS into database.
                Vector insertionresult = insertpart.sapPartInsertion(xlsfile, conn, user_id);
                request.setAttribute("insertionresult", insertionresult);
                request.setAttribute("validate_type", "sap");
                ...
                if (multi.getParameter("flag").equalsIgnoreCase("cataFlag") && multi.getParameter("flag")!=null) {
                    ...
                    result += "<br><font class='red-for-temmplate-link' ><a href=\"javascript:location.href='" + context + "'/authJSP/EAMG_Part/EAMG_Upload_SAP_Part.jsp';\">Try Again...</a></font>";
                } else {
                    result = "Error occured while reading Excel. It may be due to corrupted Excel file or invalid file path.<br> Hence Part Creation Process aborted. Attach valid Excel to complete the Process successfully.";
                    result += "<br><font class='red-for-temmplate-link' ><a href=\"javascript:location.href='" + context + "'/masterAction.do?option=initUploadSAPPartMaster';\">Try Again...</a></font>";

                }
                request.setAttribute("result", result);
                request.setAttribute("comp_type", "sap");
                //delete the temporary excel file
                if (xlsfile.exists() && xlsfile.isFile()) {
                    xlsfile.delete();
                }

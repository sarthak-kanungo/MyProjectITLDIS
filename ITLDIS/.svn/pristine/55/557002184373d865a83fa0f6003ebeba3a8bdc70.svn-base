...
//        var objSpecExp = /['&\\@\!\~\`\#\$\%\(\)\=\{\}\[\]\|\:\;\<\>\,\.\?\+\"\^\*/]/g;
        ...
        if (strObject.id == 'vendorCode') {
            text = 'Vendor Code';
          
        }
        if (strObject.id == 'vendorDesc') {
        ...
        else if (strObject.id == 'vendorCode') {
            text = 'Vendor Code';
    ...
    function cUDAction(str, row, strVal ,vendor_id)
    {

        var url = null;
        
        if (str == 'add') {
            var elementArr = new Array('vendorCode', 'vendorDesc');
            var strValue = null;
            var strObject = null;
            ...
            var vendorCode = TrimAll(document.getElementById("vendorCode").value);
            var vendorDesc = TrimAll(document.getElementById("vendorDesc").value);
            var tempDes=replaceall(vendorDesc,'&','`');
            url = "<%=cntxpath%>/masterAction.do?option=manageVendorCodeMaster&type=" + str + "&vendorCode=" + vendorCode + "&vendorDesc=" + vendorDesc + "&primary_id="+vendor_id+"&t=" + new Date().getTime();
        }
        else {
            url = "<%=cntxpath%>/masterAction.do?option=manageVendorCodeMaster&status=" + str + "&type=" + str + "&vendorCode=" + strVal + "&primary_id="+vendor_id+"&t=" + new Date().getTime();
            ...
            if (xmlHttp.readyState == 4 || xmlHttp.readyState == "complete")
            {
                ...
                else
                {
                    ...
                    if (str != 'add' && res.split("@@")[0] == 'Success') {
                        if (str == 'Y') {
                            document.getElementById('st' + row).innerHTML = "<a href=\"javascript:void(0)\" onclick=\"cUDAction('N','" + row + "','" + strVal + "','"+vendor_id+"')\"  class=\"headingSpas\" style=\"color: blue;\" >N</a>";
                        } else {
                            document.getElementById('st' + row).innerHTML = "<a href=\"javascript:void(0)\" onclick=\"cUDAction('Y','" + row + "','" + strVal + "','"+vendor_id+"')\"  class=\"headingSpas\" style=\"color: blue;\" >Y</a>";
                        }
                    }
                    if (str == 'add' && res.split("@@")[0] == 'Success') {
                        document.getElementById("msg_saveFAILED").innerHTML = res.split("@@")[1];
    ...
    function editRow(vendorCode, row,vendorDesc,vendor_id){
        var url = "<%=cntxpath%>/masterAction.do?option=manageVendorCodeMaster&type=check&vendorCode="+vendorCode+"&t="+new Date().getTime();
        var flag = false;
        xmlHttp = GetXmlHttpObject();
        xmlHttp.onreadystatechange = function()
        {
                ...
                else
                {
                    ...
                    if(res == 'notexist'){
                       // document.getElementById('aggCode'+row).innerHTML="<input type=\"text\" name=\"aggCode\" maxlength='5' class=\"headingSpas\" id=\"newAggCode"+row+"\" style=\"width:80px\" value=\""+aggCode+"\"/>";
                        document.getElementById('vendorDesc'+row).innerHTML="<input type=\"text\" name=\"vendorDesc\" size='150' maxlength='150' class=\"headingSpas\" id=\"newVendorDesc"+row+"\" style=\"width:450px\" value=\""+vendorDesc+"\"/>";
                        flag = true;
                    }else{
                        document.getElementById('vendorDesc'+row).innerHTML="<input type=\"text\" name=\"vendorDesc\" size='150' maxlength='150' class=\"headingSpas\" id=\"newVendorDesc"+row+"\" style=\"width:450px\" value=\""+vendorDesc+"\"/>";
                    }
        ...
        xmlHttp.send(null);
        document.getElementById('imageButton'+row).innerHTML="<input type=button class=\"headingSpas\" name=\"apply\" value=\"Apply\" onclick=\"updateAction('edit', "+row+",'"+vendorCode+"','"+flag+"','"+vendor_id+"')\"/>";

    }

        function updateAction(str, row, oldVendorCode,flag,vendor_id)
        {
            var newVendorCode='';
            var objRegExp = /^(\s*)$/g;
           // var objSpecExp = /['&\@\!\~\`\$\%\=\{\}\|\:\;\<\>\?\+\"\^]/g;
...
//                }
            ...
            if (document.getElementById("newVendorDesc" + row).value == "") {
                document.getElementById("newVendorDesc" + row).focus();
                document.getElementById("msg_saveFAILED").style.display = "";
                document.getElementById("msg_saveFAILED").innerHTML = "Please Enter Vendor Description";
                return false;
            }
            var chckValue = document.getElementById("newVendorDesc" + row).value;
            chckValue = chckValue.replace(objRegExp, '');
            var temp = objSpecExp.exec(chckValue);
            if (temp)
            {
                document.getElementById("newVendorDesc" + row).focus();
                document.getElementById("msg_saveFAILED").style.display = "";
                document.getElementById("msg_saveFAILED").innerHTML = "Special Characters are not allowed in Vendor Description field";
                window.scrollTo(0, 0);//go to top of page
                return false;
            }

            //  var item_No = TrimAll(document.getElementById("newMechanin_Code"+row).value);
            var vendorDesc = TrimAll(document.getElementById("newVendorDesc" + row).value);
             var tempDes=replaceall(vendorDesc,'&','`');

   var url = "<%=cntxpath%>/masterAction.do?option=manageVendorCodeMaster&id=" + oldVendorCode + "&vendorCode=" + newVendorCode + "&vendorDesc=" + tempDes + "&type=" + str + "&primary_id="+vendor_id+"&t=" + new Date().getTime();
                        ...
                        if (str != 'add' && res.split("@@")[0] == 'Success') {
                            ...
                            if(flag == 'true'){
                               // document.getElementById('aggCode'+row).innerHTML=""+newAggCode+"";
                                document.getElementById('vendorCode'+row).innerHTML=""+oldVendorCode+"";
                            }else{
                                document.getElementById('vendorCode'+row).innerHTML=""+oldVendorCode+"";
                            }
                            document.getElementById('vendorDesc' + row).innerHTML = "" + vendorDesc + "";
                            document.getElementById('imageButton' + row).innerHTML = "<img src=\"images/edit.gif\" onclick=\"editRow('" + oldVendorCode + "'," + row + ",'" + vendorDesc + "','"+vendor_id+"')\" title=\"Edit\"/>";
                        }
                    }
                }
            };
...
<div class="contentmain1">
    <div class="breadcrumb_container">
        <ul>
            ...
            <li class="breadcrumb_link">MANAGE VENDOR CODE MASTER</li>
    ...
    <center>
        ...
        <div class="content_right1">
            <div class="con_slidediv2" style="position: relative;width: 100%">
                <h1>MANAGE VENDOR CODE MASTER </h1>


                <table cellspacing=1 cellpadding=5 border=0 width=100% >
                    <tr height=30 bgcolor="#FFFFFF">
                        <td align= center class="headingSpas">
                            <html:form action="masterAction.do?option=initVendorCodeMaster" method="post" styleId="searchBy" onsubmit="donotsubmit();">
                                <table width=100%  border=0 cellspacing=0 cellpadding=0 bgcolor=#cccccc>
                                    <tr bgcolor="#FFFFFF">

                                        <td  style="padding-left:10px" width="100px" class="headingSpas" nowrap align="left">Vendor Code</td>
                        ...
                        <c:if test="${empty vendorCodeList}">
                        <tr bgcolor="#FFFFFF">
                        ...
                        <tr height=25 bgcolor="#FFFFFF">
                            <td align= center class="headingSpas">
                                <pg:pager url="masterAction.do" maxIndexPages="10" maxPageItems="10">
                                    <pg:param name="option" value="initVendorCodeMaster"/>
                                    ...
                                    <table  id="data" width=100%  border=0 cellspacing=1 cellpadding=1 bordercolor=#cccccc bgcolor=#cccccc>
                                        <tr bgcolor="#eeeeee" height="20">
                                            ...
                                            <td   align="left" class="headingSpas" width="150"  style="padding-left:3px;FONT-SIZE: 11px;font-weight:bold;padding-right:3px;"  nowrap>Vendor Code</td>
                                            <td   align="left" class="headingSpas"  style="padding-left:3px;FONT-SIZE: 11px;font-weight:bold;padding-right:3px;"  nowrap>Vendor Description</td>
                                        ...
                                        <logic:iterate id="vendorCodeList" name="vendorCodeList">
                                            <pg:item>
                                                <tr id ="${sno}" height="20">
                                                    <td align="center" bgcolor="#FFFFFF" width="25" class="headingSpas" style="padding-left:5px; padding-right: 5px">${sno}</td>
                                                    <td align="left" bgcolor="#FFFFFF" title=" ${vendorCodeList.vendorCode}" width="150" class="headingSpas" style="padding-left:5px; padding-right: 5px">
                                                        <span id ="vendorCode${sno}" >
                                                            ${vendorCodeList.vendorCode}
                                                        </span>
                                                    </td>
                                                    <td align="left" bgcolor="#FFFFFF" title="${vendorCodeList.vendorDesc}"  class="headingSpas" style="padding-left:5px; padding-right: 5px">
                                                        <span id ="vendorDesc${sno}" >
                                                            ${vendorCodeList.vendorDesc}
                                                        </span>
                                                    </td>
                                                    ...
                                                    <td align="center" bgcolor="#FFFFFF" width="20" class="headingSpas" style="padding-left:5px; padding-right: 5px">
                                                        <span id ="imageButton${sno}" >
                                                            <img alt="" src="${pageContext.request.contextPath}/images/edit.gif" onclick="editRow('${vendorCodeList.vendorCode}', ${sno}, '${vendorCodeList.vendorDesc}','${vendorCodeList.primary_id}','${vendorCodeList.id}@@${vendorCodeList.name}')" title="Edit"/>
                                                        </span>
                                                    </td>
                                                    <td align="center" bgcolor="#FFFFFF" width="25" class="headingSpas" style="padding-left:5px; padding-right: 5px">
                                                        <span id ="st${sno}" >
                                                            <a href="javascript:void(0)" onclick="cUDAction('${vendorCodeList.status}', ${sno}, '${vendorCodeList.vendorCode}','${vendorCodeList.primary_id}')"  class="headingSpas" style="color: blue;" >
                                                                ${vendorCodeList.status}&nbsp;
                                                            </a>
                                                        </span>
                                                    </td>
                                                </tr>
                                    ...
                                    <tr>
                                        <td width="100%" valign="top">
                                            <table width="100%" border="0" cellpadding="0" cellspacing="0" >
                                                ...
                                                <tr height="30">
                                                    <td  style="padding-left:10px" bgcolor="#FFFFFF" class="headingSpas" nowrap align="left">Vendor Code</td>
                                                    <td style="padding-left:10px;padding-top: 5px" bgcolor="#FFFFFF" align="left">
                                                        <input type="text" name="vendorCode"  id="vendorCode" class="headingSpas" onkeypress="return CommentsMaxLength(this, 8);" maxlength="8" style="width:120px" onblur="this.value=TrimAll(this.value)"/>
                                                    </td>

                                                    <td  style="padding-left:10px" bgcolor="#FFFFFF" class="headingSpas" nowrap align="left">Vendor Description</td>
                                                    <td style="padding-left:10px;padding-top: 5px" bgcolor="#FFFFFF" align="left">
                                                        <input type="text" name="vendorDesc"  id="vendorDesc" class="headingSpas" size="150"  maxlength="150" style="width:240px" onkeypress="return CommentsMaxLength(this, 200);" onblur="this.value=TrimAll(this.value)"/>
                                                    </td>
                                                </tr>
                                                <tr height="20">

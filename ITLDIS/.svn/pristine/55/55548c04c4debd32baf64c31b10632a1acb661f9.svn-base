<%--
    Document   : v_jobCardStatus
    Created on : Apr 30, 2014, 10:25:09 AM
...
<script language="JavaScript" type="text/javascript">
    ...
    function cUDAction(str, row,strVal,statusDesc)
    {

        if(str == 'add'){
            var elementArr=new Array('Status Desc');
            var strValue=null;
            var strObject=null;
            ...
            var name = TrimAll(document.getElementById("Status Desc").value);
            var url="<%=cntxpath%>/masterAction.do?option=manageJobCardStatus&type="+str+"&name="+name+"&t="+new Date().getTime()+"&type="+str;
        }
        else{
            var url="<%=cntxpath%>/masterAction.do?option=manageJobCardStatus&status="+str+"&statusDesc="+statusDesc+"&type="+str+"&id="+strVal+"&t="+new Date().getTime();
        }
        //alert(url);
            ...
            if (xmlHttp.readyState == 4 || xmlHttp.readyState == "complete")
            {
                ...
                else
                {
                    ...
                    if(str != 'add' && res.split("@@")[0] =='Success'){
                        if(str=='Y'){
                            document.getElementById('st'+row).innerHTML="<a href=\"javascript:void(0)\" onclick=\"cUDAction('N','"+row+"','"+strVal+"','"+statusDesc+"')\"  class=\"headingSpas\" style=\"color: blue;\" >N</a>";
                        }
                        else if(str=='N'){
                            document.getElementById('st'+row).innerHTML="<a href=\"javascript:void(0)\" onclick=\"cUDAction('Y','"+row+"','"+strVal+"','"+statusDesc+"')\"  class=\"headingSpas\" style=\"color: blue;\" >Y</a>";
                        }
                    }
                    if(str=='Y' && res.split("@@")[0] =='StatusExist' ){
                        document.getElementById('st'+row).innerHTML="<a href=\"javascript:void(0)\" onclick=\"cUDAction('Y','"+row+"','"+strVal+"','"+statusDesc+"')\"  class=\"headingSpas\" style=\"color: blue;\" >Y</a>";
                    }
                    if(str == 'add' && res.split("@@")[0] =='Success'){
                        //submitForm();
    ...
    function updateAction(str, row,id)
    {
        if(document.getElementById("newName"+row).value==""){
            ...
            document.getElementById("msg_saveFAILED").style.display = "";
            document.getElementById("msg_saveFAILED").innerHTML="Please enter value in Status Description field";
            return false;
        }
        ...
        if(temp)
        {
            document.getElementById("newName"+row).focus();
            document.getElementById("msg_saveFAILED").style.display = "";
            document.getElementById("msg_saveFAILED").innerHTML="Special Characters are not allowed in Status Description field";
            window.scrollTo(0,0);//go to top of page
            return false;
        }
        
        var name = TrimAll(document.getElementById("newName"+row).value);
        var url="<%=cntxpath%>/masterAction.do?option=manageJobCardStatus&id="+id+"&name="+name+"&type="+str+"&t="+new Date().getTime();
...
<div class="contentmain1">
    <div class="breadcrumb_container">
        <ul>
            <li class="breadcrumb_link"><a href='<%=cntxpath%>/masterAction.do?option=initOptions'>MASTERS</a>  JOB CARD STATUS</li>
                ...
                <table cellspacing=1 cellpadding=5 border=0 width=100%>
                    ...
                    <tr height=30 bgcolor="#FFFFFF">
                        <td align= center class="headingSpas">
                            <html:form action="masterAction.do?option=initJobCardStatus&param=search" method="post" styleId="searchBy" onsubmit="donotsubmit();">
                                <table width=100%  border=0 cellspacing=0 cellpadding=0 bgcolor=#cccccc>
                                    <tr bgcolor="#FFFFFF">

                                        <td  style="padding-left:10px" width="20%" class="headingSpas" nowrap align="left">Status Description</td>
                        ...
                        <c:if test="${!empty JobCardStatusList}">
                        <tr height=25 bgcolor="#FFFFFF">
                            <td align= center class="headingSpas">
                                <pg:pager url="masterAction.do" maxIndexPages="10" maxPageItems="10">
                                    <pg:param name="option" value="initJobCardStatus"/>
                                    ...
                                    <table  id="data" width=100%  border=0 cellspacing=1 cellpadding=1 bordercolor=#cccccc bgcolor=#cccccc>
                                        <tr  bgcolor="#eeeeee" height="20">
                                            ...
                                            <td   align="left" width="250"  class="headingSpas" nowrap><b>Status Description</b></td>
                                            <td   align="center" width="75" class="headingSpas" nowrap><b>Edit</b></td>
                                            <td   align="center" width="35" class="headingSpas" nowrap><b>isActive</b></td>
                                        </tr>
                                        <c:set var="sno" value="1"></c:set>
                                        <logic:iterate id="JobCardStatusList" name="JobCardStatusList">
                                            <pg:item>
                                                <tr id ="${sno}" height="20">
                                                    <td align="center" bgcolor="#FFFFFF" width="25" class="headingSpas" style="padding-left:5px; padding-right: 5px">${sno}</td>
                                                    <td align="left" bgcolor="#FFFFFF" title="${JobCardStatusList.name}" width="250" class="headingSpas" style="padding-left:5px; padding-right: 5px">
                                                        <span id ="compType${sno}" >
                                                            ${JobCardStatusList.name}
                                                        </span>
                                                    </td>
                                                    ...
                                                    <td align="center" bgcolor="#FFFFFF" width="40" class="headingSpas" style="padding-left:5px; padding-right: 5px">
                                                        <span id ="imageButton${sno}" >
                                                            <img src="${pageContext.request.contextPath}/images/edit.gif" onclick="editRow('${JobCardStatusList.name}', ${sno},'${JobCardStatusList.id}')" title="Edit"/>
                                                        </span>
                                                    </td>
                                                    <td align="center" bgcolor="#FFFFFF" width="35" class="headingSpas" style="padding-left:5px; padding-right: 5px">
                                                        <span id ="st${sno}" >
                                                            <a href="javascript:void(0)" onclick="cUDAction('${JobCardStatusList.status}', ${sno},'${JobCardStatusList.id}','${JobCardStatusList.name}')"  class="headingSpas" style="color: blue;" >
                                                                ${JobCardStatusList.status}&nbsp;
                                                            </a>
                                                        </span>
                                                    </td>
                                                </tr>
                        ...
                        <c:if test="${empty JobCardStatusList}">
                            <tr bgcolor="#FFFFFF">
                                <td valign="top" class="headingSpas" style="padding-top:10px;color: red">
                                    ...
                                    Status Description '<%=request.getParameter("nameSrch") == null ? "" : request.getParameter("nameSrch")%>' not found
                                    <%}%>
                                </td>
                            </tr>
                        </c:if>
                                            ...
                                            <table width="100%" border="0" cellpadding="0" cellspacing="0" >
                                                <tr height="30" bgcolor="#FFFFFF">
                                                    ...
                                                    <td style="padding-left:10px;padding-top:5px" width="30%">
                                                        <input type="text" name="name"  id="Status Desc" class="headingSpas" onkeypress="return CommentsMaxLength(this, 200);" maxlength="200" style="width:170px" onblur="this.value=TrimAll(this.value)"/>
                                                    </td>

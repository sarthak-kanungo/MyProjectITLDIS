/*
 * EAMG_EAMGPart_OEMPartAction.java
 *
 * Created on November 11, 2011, 12:41 PM
 */
/*
File Name: 	EAMG_EAMGPart_OEMPartAction.java
...
public class EAMG_EAMGPart_OEMPartAction extends Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm  form,
        ...
        try
        {
            ...
            xls_validate_result=new EAMG_ValidateXlsWithTemplate().isAMWPart_OEMPartExcelValidated(xlsfile,conn);
            //if validation successful.
            if(xls_validate_result.equals("success"))
            {
                //insertion of PARTS into database.
                Vector insertionresult=new EAMG_EAMGPart_OEMPartDAO().AMWPartOEMpartInsertion(xlsfile,conn,user_id);
                session.setAttribute("insertionresult",insertionresult);
            ...
            {
                result = "Error occured while reading Excel. It may be due to corrupted Excel file or invalid file path.<br> Hence AMW PART AND OEM_PART Creation Process aborted. Attach valid Excel to complete the Process successfully.";
                result+="<br><font class='red-for-temmplate-link' ><a href=\"javascript:parent.right.location.href='/authJSP/other/amw_AMW_PART_OEM_PART_CREATE.jsp';\">Try Again...</a></font>";
                request.setAttribute("result",result);
                request.setAttribute("comp_type", "AMW_PART_OEM_PART");
                //delete the temporary excel file 
                if (xlsfile.exists() && xlsfile.isFile()) 
                {
                    xlsfile.delete();
            ...
            {
                request.setAttribute("validate_type", "AMW_PART_OEM_PART");

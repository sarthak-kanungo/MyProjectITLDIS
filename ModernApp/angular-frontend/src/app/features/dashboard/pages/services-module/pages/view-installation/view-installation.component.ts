...
import { ViewInstallation, ServicesApiService } from '../../services-api.service';

@Component({
  selector: 'app-view-installation',
  standalone: true,
  ...
  template: `
    <div class="view-pdi-container">
      ...
      <div class="breadcrumb_container">
        <span class="breadcrumb_link"><a href="#">SERVICE</a> > VIEW INSTALLATION</span>
      </div>
      ...
      <div class="content-wrapper">
        ...
        <div class="header-bar">
           ...
           <div class="header-title">VIEW INSTALLATION</div>
        </div>
          ...
          <table class="data-table" cellspacing="1" cellpadding="4" width="100%" bgcolor="#cccccc">
            <thead>
              <tr bgcolor="#eeeeee" class="grid-header">
                ...
                <!-- Installation No Sort & Filter -->
                <th width="15%" align="left" (click)="sort('insNo')" class="sortable">
                   <div class="th-content">
                      <span class="col-title">Installation No. <span *ngIf="sortColumn === 'insNo'">{{ sortDirection === 'asc' ? '▲' : '▼' }}</span></span>
                      <mat-icon class="search-icon" (click)="toggleFilter('insNo', $event)">search</mat-icon>
                   </div>
                   <div class="filter-popup" *ngIf="visibleFilters['insNo']" (click)="$event.stopPropagation()">
                      <input type="text" class="filter-input" [value]="filterValues['insNo']" (keyup)="applyFilter($event, 'insNo')" placeholder="Search Inst No...">
                   </div>
                </th>

                <!-- Inst Date Sort & Filter -->
                <th width="10%" align="center" (click)="sort('insDate')" class="sortable">
                   <div class="th-content">
                      <span class="col-title">Inst. Date <span *ngIf="sortColumn === 'insDate'">{{ sortDirection === 'asc' ? '▲' : '▼' }}</span></span>
                      <mat-icon class="search-icon" (click)="toggleFilter('insDate', $event)">search</mat-icon>
                   </div>
                   <div class="filter-popup" *ngIf="visibleFilters['insDate']" (click)="$event.stopPropagation()">
                      <input type="text" class="filter-input" [value]="filterValues['insDate']" (keyup)="applyFilter($event, 'insDate')" placeholder="Search Date...">
                   </div>
                </th>

                <!-- Chassis No Sort & Filter -->
                <th width="15%" align="left" (click)="sort('vinNo')" class="sortable">
                   <div class="th-content">
                ...
                <th width="10%" align="left" (click)="sort('modelFamily')" class="sortable">
                   <div class="th-content">
                ...
                <th width="15%" align="left" (click)="sort('customerName')" class="sortable">
                   <div class="th-content">
                ...
                <!-- Mobile No Sort & Filter -->
                <th width="10%" align="left" (click)="sort('mobilePh')" class="sortable">
                   <div class="th-content">
                      <span class="col-title">Mobile No. <span *ngIf="sortColumn === 'mobilePh'">{{ sortDirection === 'asc' ? '▲' : '▼' }}</span></span>
                ...
                <!-- Dealer Name Sort & Filter -->
                <th width="10%" align="left" (click)="sort('dealerName')" class="sortable">
                   <div class="th-content">
                      <span class="col-title">Dealer Name <span *ngIf="sortColumn === 'dealerName'">{{ sortDirection === 'asc' ? '▲' : '▼' }}</span></span>
                      <mat-icon class="search-icon" (click)="toggleFilter('dealerName', $event)">search</mat-icon>
                   </div>
                   <div class="filter-popup" *ngIf="visibleFilters['dealerName']" (click)="$event.stopPropagation()">
                      <input type="text" class="filter-input" [value]="filterValues['dealerName']" (keyup)="applyFilter($event, 'dealerName')" placeholder="Search Dealer...">
                   </div>
                </th>

                <!-- Location Sort & Filter -->
                <th width="10%" align="left" (click)="sort('location')" class="sortable">
                   <div class="th-content">
                      <span class="col-title">Location <span *ngIf="sortColumn === 'location'">{{ sortDirection === 'asc' ? '▲' : '▼' }}</span></span>
                      <mat-icon class="search-icon" (click)="toggleFilter('location', $event)">search</mat-icon>
                   </div>
                   <div class="filter-popup" *ngIf="visibleFilters['location']" (click)="$event.stopPropagation()">
                      <input type="text" class="filter-input" [value]="filterValues['location']" (keyup)="applyFilter($event, 'location')" placeholder="Search Location...">
                   </div>
                </th>
              </tr>
            </thead>
            <tbody>
              <tr *ngFor="let element of filteredDataSource; let i = index" bgcolor="#FFFFFF" class="grid-row">
                ...
                <td align="left">
                  <a href="javascript:void(0)" class="link">{{element.insNo}}</a>
                </td>
                <td align="center">{{ element.insDate }}</td>
                <td align="left">{{ element.vinNo }}</td>
                <td align="left">{{ element.modelFamily }}</td>
                <td align="left">{{ element.customerName }}</td>
                <td align="left">{{ element.mobilePh }}</td>
                <td align="left">{{ element.dealerName }}</td>
                <td align="left">{{ element.location }}</td>
              </tr>
              <tr *ngIf="filteredDataSource.length === 0 && !loading" bgcolor="#FFFFFF">
                <td colspan="9" align="center" style="color:red; padding:10px;">No Record Found</td>
              </tr>
              <tr *ngIf="loading" bgcolor="#FFFFFF">
                <td colspan="9" align="center" style="padding:10px;">Loading...</td>
              </tr>
            </tbody>
          </table>
...
export class ViewInstallationComponent implements OnInit, AfterViewInit {
  dataSource: ViewInstallation[] = [];
  filteredDataSource: ViewInstallation[] = [];
  ...
  fetchData() {
    this.servicesApiService.getCompletedInstallations(this.searchChassisNo).subscribe({
      next: (data) => {
      ...
      error: (err) => {
        console.error('Error fetching completed installations:', err);
        this.loading = false;
      }
    });
  }
  ...
  processData() {
    ...
    data = data.filter(item => {
      const insMatch = !this.filterValues.insNo || (item.insNo && item.insNo.toLowerCase().includes(this.filterValues.insNo.toLowerCase()));
      const dateMatch = !this.filterValues.insDate || (item.insDate && item.insDate.toString().toLowerCase().includes(this.filterValues.insDate.toLowerCase()));
      ...
      const dealerMatch = !this.filterValues.dealerName || (item.dealerName && item.dealerName.toLowerCase().includes(this.filterValues.dealerName.toLowerCase()));
      const locMatch = !this.filterValues.location || (item.location && item.location.toLowerCase().includes(this.filterValues.location.toLowerCase()));

      return insMatch && dateMatch && vinMatch && modelMatch && customerMatch && mobileMatch && dealerMatch && locMatch;
    });
  ...
  export() {
    // Generate Excel from currently filtered data (or all data if needed, but standard is current view or all)
    // Here we export the *full* set of currently filtered (result of search & filter) data, ignoring pagination
    // To match legacy behavior, it likely exports what is "found" by the search criteria.
    const dataToExport = this.dataSource.filter(item => {
      // We re-apply the filter logic here to get the full list (not just paged)
      const insMatch = !this.filterValues.insNo || (item.insNo && item.insNo.toLowerCase().includes(this.filterValues.insNo.toLowerCase()));
      const dateMatch = !this.filterValues.insDate || (item.insDate && item.insDate.toString().toLowerCase().includes(this.filterValues.insDate.toLowerCase()));
      const vinMatch = !this.filterValues.vinNo || (item.vinNo && item.vinNo.toLowerCase().includes(this.filterValues.vinNo.toLowerCase()));
      const modelMatch = !this.filterValues.modelFamily || (item.modelFamily && item.modelFamily.toLowerCase().includes(this.filterValues.modelFamily.toLowerCase()));
  ...
  private generateLegacyExcel(data: ViewInstallation[]): void {
    ...
    let html = `
      ...
      <body>
        <center>
           <h3>VIEW INSTALLATION</h3>
           <table width="100%" border="1" cellspacing="1" cellpadding="4">
             <!-- Filter Row -->
             <tr bgcolor="#eeeeee" height="30">
                <td colspan="9" align="left" bgcolor="#eeeeee" class="textPaging">
                   <span>
                      <b>Filter On :-</b> &nbsp;&nbsp;
             ...
             <tr bgcolor="#eeeeee" height="20">
                 ...
                 <th align="left">Installation No.</th>
                 <th align="center">Inst. Date</th>
      ...
      data.forEach((item, index) => {
        html += `
            <tr bgcolor="#ffffff" height="30">
               ...
               <td align="left">${item.insNo}</td>
               <td align="center">${item.insDate}</td>
               ...
               <td align="left">${item.dealerName}</td>
    ...
    a.href = url;
    a.download = 'View_Installation_List.xls';

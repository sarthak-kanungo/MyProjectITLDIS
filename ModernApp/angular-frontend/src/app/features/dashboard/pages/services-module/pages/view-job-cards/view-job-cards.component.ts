...
@Component({
  ...
  templateUrl: './view-job-cards.component.html',
  styleUrls: ['./view-job-cards.component.scss'],
  providers: [DatePipe]
})
export class ViewJobCardsComponent implements OnInit {
  ...
  constructor(private servicesApiService: ServicesApiService, private datePipe: DatePipe) { }

  ngOnInit(): void {
    const today = new Date();
    this.toDate = this.datePipe.transform(today, 'dd/MM/yyyy') || '';
    this.fromDate = this.datePipe.transform(today, 'dd/MM/yyyy') || '';

    this.fetchJobCards();
  }

  fetchJobCards(): void {
    const validFrom = this.useDateRange ? this.fromDate : '';
    const validTo = this.useDateRange ? this.toDate : '';
  ...
  processData(): void {
    ...
    // Client-side Filtering based on column headers

...
@Component({
  selector: 'app-reopen-job-card',
  standalone: true,
  imports: [CommonModule, FormsModule, MatIconModule],
  templateUrl: './reopen-job-card.component.html',
  styleUrls: ['./reopen-job-card.component.scss'],
  providers: [DatePipe]
})
export class ReopenJobCardComponent implements OnInit {
  ...
  fetchData(): void {
    // Legacy logic likely shows 'CLOSED' or specific status cards eligible for reopening.
    // Fetching 'CLOSED' job cards for this view.
    this.servicesApiService.getViewJobCards(
      ...
      error: (err) => {
        console.error('Error fetching reopen job card data', err);
      }
    });
  }
  ...
  onStatusChange(jobCard: ViewJobCardDTO, event: any): void {
    console.log('Status changed for:', jobCard.jobCardNo, 'to', event.target.value);

...
export class GenerateInvoiceComponent implements OnInit {
  ...
  fetchData(): void {
    // improved logic: Fetching 'OPEN' or 'WIP' cards usually eligible for invoice generation
    this.servicesApiService.getViewJobCards(
      '',
      '',
      'OPEN', // Assuming OPEN cards are relevant
      undefined,
      '',
      ''
    ).subscribe({
      ...
      error: (err) => {
        console.error('Error fetching generate invoice data', err);
      }
    });
  }
  ...
  processData(): void {
    ...
    data = data.filter(item => {
      ...
      return jcMatch && vinMatch && dateMatch && payeeMatch && mobileMatch;
    });
  ...
  onGenerateInvoice(card: ViewJobCardDTO): void {
    console.log('Generate Invoice for:', card.jobCardNo);

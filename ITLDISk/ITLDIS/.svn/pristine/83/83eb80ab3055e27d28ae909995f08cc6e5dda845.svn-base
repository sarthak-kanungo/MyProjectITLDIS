 ...
 * @version
 */
public class EAMG_CreateKitDataAction extends Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
        ...
        try {
            ...
            try {
                ...
                if (imgchanged.equals("yes")) {
                    if (kit_image.exists() && kit_image.length() > 0) {
                        kit_image.renameTo(new File(ecatPath + "dealer/ecat_print/temp_kit_image/" + comp_no + ".jpg"));
                        //copy kit_image from temp_kit_image to kit_image
                        File destFolder = new File(ecatPath + "dealer/ecat_print/kit_image");

                        kit_image = new File(ecatPath + "dealer/ecat_print/temp_kit_image/" + comp_no + ".jpg");
                        if (kit_image.exists()) {
                        ...
                        request.setAttribute("option", "KIT");
                        request.setAttribute("result", "Kit Image File '" + filename + "' is not valid. Hence Kit Creation Process Aborted.");
            ...
            kitno = sheet.getCell(1, 2).getContents().trim();
            kitdesc = sheet.getCell(1, 3).getContents().trim();
            kitRemarks = sheet.getCell(1, 4).getContents().trim();
            groupType = sheet.getCell(1, 5).getContents().trim();
...
//            NDP = sheet.getCell(1, 6).getContents().trim();
            ...
            pstmt.setString(1, kitno);
            pstmt.setString(2, "KIT");
            pstmt.setString(3, kitdesc);
            pstmt.setInt(4, cdNo);
            pstmt.setInt(5, patchNo);
            pstmt.setDate(6, date);
            pstmt.setString(7, " ");
            pstmt.setString(8, kitRemarks);
...
//            pstmt.setString(11, "Y");
            ...
            e.printStackTrace();
            result = "Unable To Create kit Please Contact System Administrator.";
            conn.rollback();
        } finally {
            try {
                if (pstmt != null) {

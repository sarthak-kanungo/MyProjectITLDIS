...
* Last Modified..: $Date: 2009/06/22 13:04:28 $
...
*/
	function dynCalendar(objName, callbackFunc,image_path)
	{
            //try
            {
...
//		this.imagesPath     = arguments[2] ? arguments[2] : 'http://w07/qms/js/';
		this.imagesPath     = image_path;
	...
	function dynCalendar_show()
	{
        ...
        */
		if (this.useMonthCombo) {
			...
			monthCombo = '<select name="months" class=heading onchange="' + this.objName + '.show(this.options[this.selectedIndex].value, ' + this.objName + '.currentYear)">' + monthComboOptions + '</select>';
		} else {
        ...
        */
		if (this.useYearCombo) {
			...
			yearCombo = '<select style="border: 1px groove" class=heading name="years" onchange="' + this.objName + '.show(' + this.objName + '.currentMonth, this.options[this.selectedIndex].value)">' + yearComboOptions + '</select>';
		} else {
...
*/
	function dynCalendar_writeHTML()
	{
		...
		if (is_ie5up || is_nav6up || is_gecko) {
			document.write('<a href="javascript: ' + this.objName + '.show()"><img src="' + this.imagesPath + 'calender.png" border="0" width="20" height="22" /></a>');

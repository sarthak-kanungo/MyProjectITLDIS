 ...
 *    1.    createTemplateWithData       Avinash Pandey     1.0         to create templates with data.
 *    2.    getCompParamValue            Avinash Pandey     1.0         to get component parameter values.
 *    3.    getComponentBom              Avinash Pandey     1.0         to get Component Bom Vector.
 *    4.    isCompExist                  Avinash Pandey     1.0         to check whether the Component Exist in CompDetails or not.
 *    5.    resizeImage                  Avinash Pandey     1.0         to resize the image.
 *    6.    createThumb                  Avinash Pandey     1.0         to create thumbnail of the image.
 *    7.    getAffectedGrps              Avinash Pandey     1.0         to get affected grps by the Component.
 *    8.    getAffectedAsms              Avinash Pandey     1.0         to get affected Asms by the Component.
 *    9.    getAffectedKits              Avinash Pandey     1.0         to get affected Kits by the Component.
 *   10.    getAffectedGrpsInEcn         Avinash Pandey     1.0         to get affected Grps in ECN.
 *   11.    getMinECNNumber              Avinash Pandey     1.0         to get minimum ECN Number.
 *   12.    getECNDetails                Avinash Pandey     1.0         to get ECN Details from database.
 *   13.    getAffectedGrpsStrFrmECNGrps Avinash Pandey     1.0         to get Affected Grps String from database.
 *   14.    getECNIdFrmECNMaster         Avinash Pandey     1.0         to get ECN Id from database.
 */
...
//            }
            ...
            if (comptype.equals("PRT")) {
                ...
                ws.addCell(label_10);
                jxl.write.Label briefD5 = new jxl.write.Label(2, 2, "PART REMARKS", mandatoryFieldsTableHeadingFormat);
                ws.addCell(briefD5);
            ...
            if (comptype.equals("KIT")) {
                ...
                jxl.write.Label briefD5 = new jxl.write.Label(2, 2, "KIT REMARKS", mandatoryFieldsTableHeadingFormat);
                ws.addCell(briefD5);
                label_10 = new jxl.write.Label(2, 3, "", textFormat_LEFT);
                ws.addCell(label_10);

                jxl.write.Label briefD6 = new jxl.write.Label(3, 2, "CATEGORY TYPE", mandatoryFieldsTableHeadingFormat);
                ws.addCell(briefD6);
                label_10 = new jxl.write.Label(3, 3, "", textFormat_LEFT);
                ws.addCell(label_10);
            }
            ...
            try {
                ...
                //rs = st.executeQuery("select part_no , p1 , p3 , p4 , np4 from part");
                //rs = st.executeQuery("select part_no , p1 , p3,np4,p4  from cat_part where part_type='" + comptype + "'");
                String query = ("select part_no , p1 , p3,np4,p4  from cat_part(NOLOCK) where part_type='" + comptype + "'");
            	st = conn.prepareStatement(query);
            	rs = st.executeQuery();
                while (rs.next()) {
                    colCounter = 0;
                    comp_no = rs.getString(1);
                    jxl.write.Label briefD3 = new jxl.write.Label(colCounter, rowCounter, comp_no, textFormat_LEFT);
                    ws.addCell(briefD3);
                    colCounter = 1;
                    comp_desc = rs.getString(2);
                    jxl.write.Label briefD4 = new jxl.write.Label(colCounter, rowCounter, comp_desc, textFormat_LEFT);
                    ws.addCell(briefD4);
                    colCounter = 2;
                    comprmk = rs.getString(3);
                    jxl.write.Label briefD5 = new jxl.write.Label(colCounter, rowCounter, comprmk, textFormat_LEFT);
                    ws.addCell(briefD5);
                    colCounter = 3;
//                    categoryType = rs.getString(4);
//                    jxl.write.Label briefD6 = new jxl.write.Label(colCounter, rowCounter, categoryType, textFormat_LEFT);
//                    ws.addCell(briefD6);
                  //  colCounter = 4;
                    serviciability = rs.getString(5);
                    jxl.write.Label briefD6 = new jxl.write.Label(colCounter, rowCounter, serviciability, textFormat_LEFT);
                    ws.addCell(briefD6);
                    colCounter = 4;
//                    partStatus = rs.getString(5);
//                    jxl.write.Label briefD7 = new jxl.write.Label(colCounter, rowCounter, partStatus, textFormat_LEFT);
//                    ws.addCell(briefD7);
//                    colCounter = 5;
                    rowCounter++;
                }
    ...
    public Vector getComponentBom(String comp_no, String comp_type, Vector bomVec, Connection conn) {
        ...
        //if (comp_type.equals("ASM")) {
            //query = "Select COMPONENT,COMP_TYPE from CAT_ASSY_BOM where ASSY_NO='" + comp_no + "'";
        //} else {
            query = "Select distinct COMPONENT,COMP_TYPE from CAT_S_KIT_BOM(NOLOCK) where KIT_NO='" + comp_no + "'";
        //}
...
//            //System.out.println("bomVec :"+bomVec);
    ...
    public String isCompExist(String comp_no, String comp_type, Connection conn) throws Exception {
        ...
        String query = "Select * from COMP_DETAIL(NOLOCK) where COMP_NO='" + comp_no + "' and COMP_TYPE='" + comp_type + "'";
...
//        //System.out.println("result :"+result);    
    ...
    public Vector getAffectedGrps(String comp_no, Connection conn) throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < affectedkits.size(); i++) {
                ...
                String query = ("select GRP_KIT_NO from CAT_GROUP_KIT_BOM(NOLOCK) where COMPONENT='" + kit + "'");
                st = conn.prepareStatement(query);
                rs = st.executeQuery();
                while (rs.next()) {
                    grp = rs.getString(1);
    ...
    public Vector getAffectedAsms(String comp_no, Vector affectedasms, Connection conn) throws Exception {
        ...
        try {
            //st = conn.createStatement();
            //rs = st.executeQuery("select ASSY_NO from CAT_ASSY_BOM(NOLOCK) where COMPONENT='" + comp_no + "'");
    ...
    public Vector getAffectedKits(String comp_no, Connection conn) throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < affectedasms.size(); i++) {
                ...
                //rs = st.executeQuery("select KIT_NO from S_KIT_BOM where COMPONENT='" + asm + "'");
                String query = ("select KIT_NO from S_KIT_BOM(NOLOCK) where COMPONENT='" + asm + "'");
                st = conn.prepareStatement(query);
                rs = st.executeQuery();
                while (rs.next()) {
                    kit = rs.getString(1);
    ...
    public Vector getInValidECNDetails(int ecn_no, String model_no, Vector invalidEcnVec, Connection conn) throws Exception {
        ...
        String query1 = "select mg.MODEL_NO,mg.GROUP_NO from cat_model_group(NOLOCK) mg,cat_group_kit_bom(NOLOCK) gkt where gkt.GRP_KIT_NO=mg.GROUP_NO and gkt.COMPONENT='P2'";
        //stmt = conn.createStatement();
        //rs = stmt.executeQuery(query);
        stmt = conn.prepareStatement(query);
        rs = stmt.executeQuery();
        while (rs.next()) {
            item = rs.getString(1);
            //stmt1 = conn.createStatement();
            query1 = "select mg.MODEL_NO,mg.GROUP_NO from cat_model_group(NOLOCK) mg,cat_group_kit_bom(NOLOCK) gkt where gkt.GRP_KIT_NO=mg.GROUP_NO and gkt.COMPONENT='" + item + "'";
            //rs1 = stmt1.executeQuery(query1);
            stmt1 = conn.prepareStatement(query1);
            rs1 = stmt1.executeQuery();
            if (!rs1.next()) {
    ...
    public Vector getGroupDetailsInECN(String grp_no, Connection conn) throws Exception {
        ...
        //stmt = conn.createStatement();
        String query = "";
        query = "select COMPONENT,QTY from group_kit_bom(NOLOCK) where GRP_KIT_NO='" + grp_no + "'";
        //rs = stmt.executeQuery(query);
        stmt = conn.prepareStatement(query);
        rs = stmt.executeQuery();
        while (rs.next()) {
    ...
    public boolean createTemplateWithDataPart(String functionality_type, String path, Connection conn) {
        ...
        int colCounter = 0, rowCounter = 0;
        WritableWorkbook workbook = null;
        String comp_no = "",comp_desc = "",compNo = "",compDesc="";
        String filepath = path+"/"+ functionality_type + "_template.xls";
        String fields="",table_name="";
        File file = new File(filepath);
        if (file.exists()) {
            file.delete();
        }
        try {
            ...
            textFormat_LEFT.setAlignment(Alignment.LEFT);
            textFormat_LEFT.setVerticalAlignment(VerticalAlignment.CENTRE);
            textFormat_LEFT.setBackground(Colour.WHITE);
            textFormat_LEFT.setBorder(Border.ALL, BorderLineStyle.THIN);
...
//				tableHeadingFormat.setBorder(Border.ALL, BorderLineStyle.MEDIUM);
            ...
            if (functionality_type.equals("PART_VS_ALTERNATE")) {
                fields="ALTERNATE_PART_NO";
                table_name="CAT_ALTERNATE_PART_MASTER";
                jxl.write.Label briefD1 = new jxl.write.Label(0, 0, "PART VS ALTERNATE PART TEMPLATE", headingFormat);
                ws.mergeCells(0, 0, 1, 0);
                ws.addCell(briefD1);
                jxl.write.Label briefD3 = new jxl.write.Label(0, 2, "PART NUMBER", mandatoryFieldsTableHeadingFormat);
                ws.addCell(briefD3);
                ...
                label_10 = new jxl.write.Label(1, 3, "", textFormat_LEFT);
                ws.addCell(label_10);
                jxl.write.Label briefD5 = new jxl.write.Label(2, 2, "ALTERNATE NUMBER", mandatoryFieldsTableHeadingFormat);
                ws.addCell(briefD5);
                label_10 = new jxl.write.Label(2, 3, "", textFormat_LEFT);
                ws.addCell(label_10);
                jxl.write.Label briefD6 = new jxl.write.Label(3, 2, "ALTERNATE DESCRIPTION", mandatoryFieldsTableHeadingFormat);
                ws.addCell(briefD6);
                label_10 = new jxl.write.Label(3, 3, "", textFormat_LEFT);
                ws.addCell(label_10);
                jxl.write.Label briefD7 = new jxl.write.Label(4, 2, "SET", mandatoryFieldsTableHeadingFormat);
                ws.addCell(briefD7);
            ...
            else if (functionality_type.equals("PART_VS_PAINT")) {
                fields="PAINT_CODE";
                table_name="CAT_PAINT_CODE_MASTER";
                jxl.write.Label briefD1 = new jxl.write.Label(0, 0, "PART VS PAINT CODE TEMPLATE", headingFormat);
                ws.mergeCells(0, 0, 1, 0);
                ws.addCell(briefD1);
                ...
                jxl.write.Label briefD5 = new jxl.write.Label(2, 2, " PAINT CODE", mandatoryFieldsTableHeadingFormat);
                ws.addCell(briefD5);
                label_10 = new jxl.write.Label(2, 3, "", textFormat_LEFT);
                ws.addCell(label_10);

                jxl.write.Label briefD6 = new jxl.write.Label(3, 2, "PAINT CODE DESCRIPTION", mandatoryFieldsTableHeadingFormat);
                ws.addCell(briefD6);
                label_10 = new jxl.write.Label(3, 3, "", textFormat_LEFT);
                ws.addCell(label_10);
                colCounter = 5;
            ...
            try {
               ...
               if (functionality_type.equals("PART_VS_ALTERNATE")) {                   
                    //rs = st.executeQuery("select distinct ppm.part_no,(select p1 from CAT_PART where part_no=ppm.part_no),ppm." + fields + " ,(select p1 from CAT_PART where part_no=ppm." + fields + "), ppm.SET_NO from  " + table_name + " ppm ");
                String query = ("select distinct ppm.part_no,(select p1 from CAT_PART(NOLOCK) where part_no=ppm.part_no),ppm." + fields + " ,(select p1 from CAT_PART(NOLOCK) where part_no=ppm." + fields + "), ppm.SET_NO from  " + table_name + " ppm ");
                st = conn.prepareStatement(query);
                rs = st.executeQuery();
            	   while (rs.next()) {
                    colCounter = 0;
                    ...
                    ws.addCell(briefD4);
                    colCounter = 2;
                    compNo = rs.getString(3);
                    jxl.write.Label briefD5 = new jxl.write.Label(colCounter, rowCounter, compNo, textFormat_LEFT);
                    ws.addCell(briefD5);
                    colCounter = 3;
                    compDesc = rs.getString(4);
                    jxl.write.Label briefD6 = new jxl.write.Label(colCounter, rowCounter, compDesc, textFormat_LEFT);
                    ws.addCell(briefD6);
                    colCounter = 4;                  
                    compDesc = rs.getString(5);
                    jxl.write.Label briefD7 = new jxl.write.Label(colCounter, rowCounter, compDesc, textFormat_LEFT);
                    ws.addCell(briefD7);
                    colCounter = 5;                  
                    rowCounter++;
                 }
                } else {
                    //rs = st.executeQuery("select distinct ppm.part_no,(select p1 from CAT_PART where part_no=ppm.part_no),ppm." + fields + " ,(select p1 from CAT_PART where part_no=ppm." + fields + ") from  " + table_name + " ppm ");
               String query1 = ("select distinct ppm.part_no,(select p1 from CAT_PART(NOLOCK) where part_no=ppm.part_no),ppm." + fields + " ,(select p1 from CAT_PART(NOLOCK) where part_no=ppm." + fields + ") from  " + table_name + " ppm ");
            ...
            label_11 = new jxl.write.Label(0, rowCounter, "", textFormat_LEFT);
            rowCounter++;
            jxl.write.Label label_13 = new jxl.write.Label(0, rowCounter, "");
            ws.addCell(label_13);

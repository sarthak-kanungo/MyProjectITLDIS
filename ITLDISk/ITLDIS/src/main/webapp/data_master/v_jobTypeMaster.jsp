...
<script language="JavaScript" type="text/javascript">
    ...
    function cUDAction(str, row,strVal,cTypeName,flag)
    {

        if(str == 'add'){
            var elementArr=new Array('Job Type');
            var strValue=null;
            var strObject=null;
            ...
            for(var i=0;i<elementArr.length;i++)
            {
                ...
                {  return false;}
                <%--else{
                    var subvalue=strValue.substring(0, 1);
                    var iChars = "0123456789";

                    for (var i = 0; i < subvalue.length; i++) {
                        if (iChars.indexOf(subvalue.charAt(i)) != -1){
                            strObject.focus();
                            document.getElementById("msg_saveFAILED").style.display = "";
                            document.getElementById("msg_saveFAILED").innerHTML=strObject.id+" shouldn't start with numbers";
                            window.scrollTo(0,0);//go to top of page
                            return false;
                        }
                    }
            ...
            var name = TrimAll(document.getElementById("Job Type").value).replace('&','@@');
            var url="<%=cntxpath%>/masterAction.do?option=manageJobType&type="+str+"&name="+name+"&t="+new Date().getTime();
        }
        else if(flag=='freeService'){
             var url="<%=cntxpath%>/masterAction.do?option=manageJobType&status="+str+"&jobTypeName="+cTypeName+"&name="+name+"&type="+flag+"&id="+strVal+"&flag="+flag+"&t="+new Date().getTime();
        }
        else{
            var url="<%=cntxpath%>/masterAction.do?option=manageJobType&status="+str+"&jobTypeName="+cTypeName+"&name="+name+"&type="+str+"&id="+strVal+"&t="+new Date().getTime();
        }
        //alert(url);
        ...
        xmlHttp.onreadystatechange = function()
        {
            if (xmlHttp.readyState == 4 || xmlHttp.readyState == "complete")
            {
                ...
                else
                {
                    ...
                    if(flag == 'freeService' && res.split("@@")[0] =='Success'){
                        if(str=='Y'){
                            document.getElementById('fs'+row).innerHTML="<a href=\"javascript:void(0)\" onclick=\"cUDAction('N','"+row+"','"+strVal+"','"+cTypeName+"','freeService')\"  class=\"headingSpas\" style=\"color: blue;\" >N</a>";
                        }
                        else if(str=='N'){
                            document.getElementById('fs'+row).innerHTML="<a href=\"javascript:void(0)\" onclick=\"cUDAction('Y','"+row+"','"+strVal+"','"+cTypeName+"','freeService')\"  class=\"headingSpas\" style=\"color: blue;\" >Y</a>";
                        }
                    }
                   
                    if(str=='Y' && res.split("@@")[0] =='JobTypeExist' ){
                        document.getElementById('st'+row).innerHTML="<a href=\"javascript:void(0)\" onclick=\"cUDAction('Y','"+row+"','"+strVal+"','"+cTypeName+"')\"  class=\"headingSpas\" style=\"color: blue;\" >Y</a>";
                    }
                    if(str == 'add' && res.split("@@")[0] =='Success'){
                        //submitForm();
    ...
    function updateAction(str, row,id)
    {
        if(document.getElementById("newName"+row).value==""){
            ...
            document.getElementById("msg_saveFAILED").style.display = "";
            document.getElementById("msg_saveFAILED").innerHTML="Please Enter Value in Job Type Description Field";
            return false;
        }
        ...
        if(temp)
        {
            document.getElementById("newName"+row).focus();
            document.getElementById("msg_saveFAILED").style.display = "";
            document.getElementById("msg_saveFAILED").innerHTML="Special Characters are not allowed in Job Type Description Field";
            window.scrollTo(0,0);//go to top of page
            return false;
        }
        <%--else{
            var subvalue=document.getElementById("newName"+row).value.substring(0, 1);
            var iChars = "0123456789";
            ...
            for (var i = 0; i < subvalue.length; i++) {
                if (iChars.indexOf(subvalue.charAt(i)) != -1){
                    document.getElementById("newName"+row).focus();
                    document.getElementById("msg_saveFAILED").style.display = "";
                    document.getElementById("msg_saveFAILED").innerHTML="Job Type shouldn't start with numbers";
                    window.scrollTo(0,0);//go to top of page
                    return false;
                }
            }
        ...
        var url="<%=cntxpath%>/masterAction.do?option=manageJobType&id="+id+"&name="+name+"&type="+str+"&t="+new Date().getTime();
...
<div class="contentmain1">
    <div class="breadcrumb_container">
        <ul>
            <li class="breadcrumb_link"><a href='<%=cntxpath%>/masterAction.do?option=initOptions'>MASTERS</a>  MANAGE JOB TYPE</li>
                ...
                <table cellspacing=1 cellpadding=5 border=0 width=100%>
                    ...
                    <tr height=30 bgcolor="#FFFFFF">
                        <td align= center class="headingSpas">
                            <html:form action="masterAction.do?option=initJobType&param=search" method="post" styleId="searchBy" onsubmit="donotsubmit();">
                                <table width=100%  border=0 cellspacing=0 cellpadding=0 bgcolor=#cccccc>
                                    <tr bgcolor="#FFFFFF">

                                        <td  style="padding-left:10px" width="20%" class="headingSpas" nowrap align="left">Job Type Description</td>
                                ...
                                <pg:pager url="masterAction.do" maxIndexPages="10" maxPageItems="10">
                                    ...
                                    <table  id="data" width=100%  border=0 cellspacing=1 cellpadding=1 bordercolor=#cccccc bgcolor=#cccccc>
                                        <tr  bgcolor="#eeeeee" height="20">
                                            <td   align="center" width="20" class="headingSpas"  ><b>S.No</b></td>
                                            <td   align="left"  width="250" class="headingSpas" ><b>Job Type Description</b></td>
                                            <td   align="center"  width="35" class="headingSpas" ><b>Free Service</b></td>
                                            <td   align="center" width="40" class="headingSpas" ><b>Edit</b></td>
                                            <td   align="center" width="35" class="headingSpas" ><b>isActive</b></td>
                                        </tr>
                                        <c:set var="sno" value="1"></c:set>
                                        <logic:iterate id="JobTypeList" name="JobTypeList">
                                            <pg:item>
                                                <tr id ="${sno}" height="20">
                                                    <td align="center" bgcolor="#FFFFFF" width="25" class="headingSpas" style="padding-left:5px; padding-right: 5px">${sno}</td>
                                                    <td align="left" bgcolor="#FFFFFF" title="${JobTypeList.name}" width="100" class="headingSpas" style="padding-left:5px; padding-right: 5px">
                                                    ...
                                                    <td align="center" bgcolor="#FFFFFF" width="25" class="headingSpas" style="padding-left:5px; padding-right: 5px">
                                                        <span id ="fs${sno}" >
                                                            <a href="javascript:void(0)" onclick="cUDAction('${JobTypeList.freeService}', ${sno},'${JobTypeList.id}','${JobTypeList.name}','freeService')"  class="headingSpas" style="color: blue;" >
                                                                ${JobTypeList.freeService}&nbsp;
                                                            </a>
                                                        </span>
                                                    </td>
                                                    <td align="center" bgcolor="#FFFFFF" width="20" class="headingSpas" style="padding-left:5px; padding-right: 5px">
                                                        <span id ="imageButton${sno}" >
                                                            <img src="${pageContext.request.contextPath}/images/edit.gif" onclick="editRow('${JobTypeList.name}', ${sno},'${JobTypeList.id}')" title="Edit"/>
                                                        </span>
                                                    </td>
                                                    <td align="center" bgcolor="#FFFFFF" width="25" class="headingSpas" style="padding-left:5px; padding-right: 5px">
                                                        <span id ="st${sno}" >
                                                            <a href="javascript:void(0)" onclick="cUDAction('${JobTypeList.status}', ${sno},'${JobTypeList.id}','${JobTypeList.name}')"  class="headingSpas" style="color: blue;" >
                                                                ${JobTypeList.status}&nbsp;
                                                            </a>
                                                        </span>
                                                    </td>
                        ...
                        <c:if test="${empty JobTypeList}">
                            <tr bgcolor="#FFFFFF">
                                <td valign="top" class="headingSpas" style="padding-top:10px;color: red">
                                    ...
                                    Job Type Description '<%=request.getParameter("nameSrch") == null ? "" : request.getParameter("nameSrch")%>' not found
                                    <%}%>
                                </td>
                            </tr>
                        </c:if>
                                            ...
                                            <table width="100%" border="0" cellpadding="0" cellspacing="0" >
                                                <tr height="30" bgcolor="#FFFFFF">
                                                    ...
                                                    <td style="padding-left:10px;padding-top:5px" width="30%">
                                                        <input type="text" name="name"  id="Job Type" class="headingSpas" size="200" maxlength="200" style="width:170px" onblur="this.value=TrimAll(this.value)"/>
                                                    </td>

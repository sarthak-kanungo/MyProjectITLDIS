...
<script language="JavaScript" type="text/javascript">
    ...
    function cUDAction(str, row,strVal,nextServiceDesc)
    {

        if(str == 'add'){
            var elementArr=new Array('Next Service Desc');
            var strValue=null;
            var strObject=null;
            ...
            var name = TrimAll(document.getElementById("Next Service Desc").value);
            var url="<%=cntxpath%>/masterAction.do?option=manageNextServiceFrom&type="+str+"&name="+name+"&t="+new Date().getTime();
        }
        else{
            var url="<%=cntxpath%>/masterAction.do?option=manageNextServiceFrom&status="+str+"&nextServiceDesc="+nextServiceDesc+"&type="+str+"&id="+strVal+"&t="+new Date().getTime();
        }
        //alert(url);
            ...
            if (xmlHttp.readyState == 4 || xmlHttp.readyState == "complete")
            {
                ...
                else
                {
                    ...
                    if(str != 'add' && res.split("@@")[0] =='Success'){
                        if(str=='Y'){
                            document.getElementById('st'+row).innerHTML="<a href=\"javascript:void(0)\" onclick=\"cUDAction('N','"+row+"','"+strVal+"','"+nextServiceDesc+"')\"  class=\"headingSpas\" style=\"color: blue;\" >N</a>";
                        }
                        else if(str=='N'){
                            document.getElementById('st'+row).innerHTML="<a href=\"javascript:void(0)\" onclick=\"cUDAction('Y','"+row+"','"+strVal+"','"+nextServiceDesc+"')\"  class=\"headingSpas\" style=\"color: blue;\" >Y</a>";
                        }
                    }
                    if(str=='Y' && res.split("@@")[0] =='nextServiceExist' ){
                        document.getElementById('st'+row).innerHTML="<a href=\"javascript:void(0)\" onclick=\"cUDAction('Y','"+row+"','"+strVal+"','"+nextServiceDesc+"')\"  class=\"headingSpas\" style=\"color: blue;\" >Y</a>";
                    }
                    if(str == 'add' && res.split("@@")[0] =='Success'){
                        //submitForm();
                        document.getElementById("msg_saveFAILED").innerHTML=res.split("@@")[1];
                        document.getElementById("Next Service Desc").value='';
                    }
                } }
        };
    ...
    function editRow(name, row,id){
        document.getElementById('compType'+row).innerHTML="<input type=\"text\" name=\"name\" size='200' maxlength='200' class=\"headingSpas\" onkeypress=\"return CommentsMaxLength(this, 200);\" id=\"newName"+row+"\" style=\"width:280px\" value=\""+name+"\"/>";
    ...
    function updateAction(str, row,id)
    {
        if(document.getElementById("newName"+row).value==""){
            ...
            document.getElementById("msg_saveFAILED").style.display = "";
            document.getElementById("msg_saveFAILED").innerHTML="Please enter in Next Service Description field";
            return false;
        }
        ...
        if(temp)
        {
            document.getElementById("newName"+row).focus();
            document.getElementById("msg_saveFAILED").style.display = "";
            document.getElementById("msg_saveFAILED").innerHTML="Special Characters are not allowed in Next Service Description field";
            window.scrollTo(0,0);//go to top of page
            return false;
        }

        var name = TrimAll(document.getElementById("newName"+row).value);
        var url="<%=cntxpath%>/masterAction.do?option=manageNextServiceFrom&id="+id+"&name="+name+"&type="+str+"&t="+new Date().getTime();
        ...
        xmlHttp.onreadystatechange = function()
        {
            if (xmlHttp.readyState == 4 || xmlHttp.readyState == "complete")
            {
                ...
                { res = trim(xmlHttp.responseText);
                    document.getElementById("msg_saveFAILED").innerHTML=res.split("@@")[1];
                    if(str != 'add' && res.split("@@")[0] =='Success'){
                        document.getElementById('compType'+row).innerHTML=""+name.toString().toUpperCase()+"";
...
<div class="contentmain1">
    <div class="breadcrumb_container">
        <ul>
            <li class="breadcrumb_link"><a href='<%=cntxpath%>/masterAction.do?option=initOptions'>MASTERS</a>  NEXT SERVICE FROM</li>
                ...
                <table cellspacing=1 cellpadding=5 border=0 width=100%>
                    ...
                    <tr height=30 bgcolor="#FFFFFF">
                        <td align= center class="headingSpas">
                            <html:form action="masterAction.do?option=initNextServiceFrom&param=search" method="post" styleId="searchBy" onsubmit="donotsubmit();">
                                <table width=100%  border=0 cellspacing=0 cellpadding=0 bgcolor=#cccccc>
                                    <tr bgcolor="#FFFFFF">

                                        <td  style="padding-left:10px" width="20%" class="headingSpas" nowrap align="left">Next Service Description</td>
                        ...
                        <c:if test="${!empty NextServiceList}">
                        <tr height=25 bgcolor="#FFFFFF">
                            <td align= center class="headingSpas">
                                <pg:pager url="masterAction.do" maxIndexPages="10" maxPageItems="10">
                                    <pg:param name="option" value="initNextServiceFrom"/>
                                    ...
                                    <table  id="data" width=100%  border=0 cellspacing=1 cellpadding=1 bordercolor=#cccccc bgcolor=#cccccc>
                                        <tr  bgcolor="#eeeeee" height="20">
                                            ...
                                            <td   align="left" width="250"  class="headingSpas" nowrap><b>Next Service Description</b></td>
                                        ...
                                        <logic:iterate id="NextServiceList" name="NextServiceList">
                                            <pg:item>
                                                <tr id ="${sno}" height="20">
                                                    <td align="center" bgcolor="#FFFFFF" width="35" class="headingSpas" style="padding-left:5px; padding-right: 5px">${sno}</td>
                                                    <td align="left" bgcolor="#FFFFFF" title="${NextServiceList.name}" width="250" class="headingSpas" style="padding-left:5px; padding-right: 5px">
                                                        <span id ="compType${sno}" >
                                                            ${NextServiceList.name}
                                                        </span>
                                                    </td>
                                                    ...
                                                    <td align="center" bgcolor="#FFFFFF" width="40" class="headingSpas" style="padding-left:5px; padding-right: 5px">
                                                        <span id ="imageButton${sno}" >
                                                            <img src="${pageContext.request.contextPath}/images/edit.gif" onclick="editRow('${NextServiceList.name}', ${sno},'${NextServiceList.id}')" title="Edit"/>
                                                        </span>
                                                    </td>
                                                    <td align="center" bgcolor="#FFFFFF" width="35" class="headingSpas" style="padding-left:5px; padding-right: 5px">
                                                        <span id ="st${sno}" >
                                                            <a href="javascript:void(0)" onclick="cUDAction('${NextServiceList.status}', ${sno},'${NextServiceList.id}','${NextServiceList.name}')"  class="headingSpas" style="color: blue;" >
                                                                ${NextServiceList.status}&nbsp;
                                                            </a>
                                                        </span>
                                                    </td>
                                                </tr>
                        ...
                        <c:if test="${empty NextServiceList}">
                            <tr bgcolor="#FFFFFF">
                                <td valign="top" class="headingSpas" style="padding-top:10px;color: red">
                                    ...
                                    Next Service Description '<%=request.getParameter("nameSrch") == null ? "" : request.getParameter("nameSrch")%>' not found
                                    <%}%>
                                </td>
                            </tr>
                        </c:if>
                                            ...
                                            <table width="100%" border="0" cellpadding="0" cellspacing="0" >
                                                <tr height="30" bgcolor="#FFFFFF">
                                                    ...
                                                    <td style="padding-left:10px;padding-top:5px" width="30%">
                                                        <input type="text" name="name"  id="Next Service Desc" size='200' class="headingSpas" onkeypress="return CommentsMaxLength(this, 200);" maxlength="200" style="width:220px" onblur="this.value=TrimAll(this.value)"/>
                                                    </td>

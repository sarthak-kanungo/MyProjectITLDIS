...
<script language="JavaScript" type="text/javascript">
    ...
    function cUDAction(str, row,strVal,billDesc)
    {

        if(str == 'add'){
            var elementArr=new Array('Bill Description');
            var strValue=null;
            var strObject=null;
            ...
            var name = TrimAll(document.getElementById("Bill Description").value).replace('&','@@');
            var percentage=TrimAll(document.getElementById("percentage").value);
...
//,'percentage'
            ...
            if(percentage==""){
                ...
                document.getElementById("msg_saveFAILED").innerHTML="Please Enter  Value in Discount% Field";
                return false;
            }

            if(isNaN(percentage)){

                document.getElementById("msg_saveFAILED").innerHTML="Please Enter Numeric Value in Discount% Field";
                return false;
            }

            if(percentage>100){

                document.getElementById("msg_saveFAILED").innerHTML="Please Enter Value less than 100 in Discount% Field";
                return false;
            ...
            if(percentage<100){
                ...
                percentage=eval(percentage).toFixed(2);


            }


            var url="<%=cntxpath%>/masterAction.do?option=manageBillMaster&type="+str+"&name="+name+"&percentage="+percentage+"&t="+new Date().getTime();
        }
        else{
            var url="<%=cntxpath%>/masterAction.do?option=manageBillMaster&status="+str+"&billDesc="+billDesc+"&name="+name+"&type="+str+"&id="+strVal+"&t="+new Date().getTime();
        }
        //alert(url);
            ...
            if (xmlHttp.readyState == 4 || xmlHttp.readyState == "complete")
            {
                ...
                else
                {
                    ...
                    if(str != 'add' && res.split("@@")[0] =='Success'){
                        if(str=='Y'){
                            document.getElementById('st'+row).innerHTML="<a href=\"javascript:void(0)\" onclick=\"cUDAction('N','"+row+"','"+strVal+"','"+billDesc+"')\"  class=\"headingSpas\" style=\"color: blue;\" >N</a>";
                        }
                        else if(str=='N'){
                            document.getElementById('st'+row).innerHTML="<a href=\"javascript:void(0)\" onclick=\"cUDAction('Y','"+row+"','"+strVal+"','"+billDesc+"')\"  class=\"headingSpas\" style=\"color: blue;\" >Y</a>";
                        }
                    }
                    if(str=='Y' && res.split("@@")[0] =='billExist' ){
                        document.getElementById('st'+row).innerHTML="<a href=\"javascript:void(0)\" onclick=\"cUDAction('Y','"+row+"','"+strVal+"','"+billDesc+"')\"  class=\"headingSpas\" style=\"color: blue;\" >Y</a>";
                    }
                    if(str == 'add' && res.split("@@")[0] =='Success'){
                        //submitForm();
    ...
    function editRow(name,percentage, row,id){
        ...
        document.getElementById('percentage'+row).innerHTML="<input type=\"text\" name=\"name\" maxlength='6' class=\"headingSpas\"  id=\"newPercentage"+row+"\" style=\"width:220px\" value=\""+percentage+"\"/>";
    ...
    function updateAction(str, row,id)
    {
        if(document.getElementById("newName"+row).value==""){
            ...
            document.getElementById("msg_saveFAILED").style.display = "";
            document.getElementById("msg_saveFAILED").innerHTML="Please enter value in Bill Description field";
            return false;
        }
        if(document.getElementById("newPercentage"+row).value==""){
            document.getElementById("newPercentage"+row).focus();
            document.getElementById("msg_saveFAILED").style.display = "";
            document.getElementById("msg_saveFAILED").innerHTML="Please enter value in Discount% field";
            return false;
        }
        ...
        if(temp)
        {
            document.getElementById("newName"+row).focus();
            document.getElementById("msg_saveFAILED").style.display = "";
            document.getElementById("msg_saveFAILED").innerHTML="Special Characters are not allowed in Bill Description field";
            window.scrollTo(0,0);//go to top of page
            return false;
        }

        var name = TrimAll(document.getElementById("newName"+row).value).replace('&','@@');
        var percentage = TrimAll(document.getElementById("newPercentage"+row).value);
        ...
        var url="<%=cntxpath%>/masterAction.do?option=manageBillMaster&id="+id+"&name="+name+"&percentage="+percentage+"&type="+str+"&t="+new Date().getTime();
            ...
            if (xmlHttp.readyState == 4 || xmlHttp.readyState == "complete")
            {
                ...
                { res = trim(xmlHttp.responseText);
                    ...
                    if(str != 'add' && res.split("@@")[0] =='Success'){
                        ...
                        document.getElementById('percentage'+row).innerHTML=""+percentage.toString().toUpperCase()+"";
                        document.getElementById('imageButton'+row).innerHTML="<img src=\"images/edit.gif\" onclick=\"editRow('"+name+"','"+percentage+"',"+row+",'"+id+"')\" title=\"Edit\"/>";
                    }
                } }
        };
        xmlHttp.open("GET",url, true);
...
<div class="contentmain1">
    <div class="breadcrumb_container">
        <ul>
            <li class="breadcrumb_link"><a href='<%=cntxpath%>/masterAction.do?option=initOptions'>MASTERS</a>BILL TYPE MASTER</li>
                ...
                <table cellspacing=1 cellpadding=5 border=0 width=100%>
                    ...
                    <tr height=30 bgcolor="#FFFFFF">
                        <td align= center class="headingSpas">
                            <html:form action="masterAction.do?option=initBillMaster&param=search" method="post" styleId="searchBy" onsubmit="donotsubmit();">
                                <table width=100%  border=0 cellspacing=0 cellpadding=0 bgcolor=#cccccc>
                                    <tr bgcolor="#FFFFFF">

                                        <td  style="padding-left:10px" width="20%" class="headingSpas" nowrap align="left">Bill Description</td>
                        ...
                        <tr height=25 bgcolor="#FFFFFF">
                            <td align= center class="headingSpas">
                                <pg:pager url="masterAction.do" maxIndexPages="10" maxPageItems="10">
                                    <pg:param name="option" value="initBillMaster"/>
                                    ...
                                    <table  id="data" width=100%  border=0 cellspacing=1 cellpadding=1 bordercolor=#cccccc bgcolor=#cccccc>
                                        ...
                                        <logic:iterate id="billList" name="billList">
                                            <pg:item>
                                                <tr id ="${sno}" height="20">
                                                    <td align="center" bgcolor="#FFFFFF" width="25" class="headingSpas" style="padding-left:5px; padding-right: 5px">${sno}</td>
                                                    <td align="left" bgcolor="#FFFFFF" title="${billList.name}" width="150" class="headingSpas" style="padding-left:5px; padding-right: 5px">
                                                        <span id ="compType${sno}" >
                                                            ${billList.name}
                                                        </span>
                                                    </td>
                                                    <td align="left" bgcolor="#FFFFFF" title="${billList.percentage}" width="150" class="headingSpas" style="padding-left:5px; padding-right: 5px">
                                                        <span id ="percentage${sno}" >
                                                            ${billList.percentage}
                                                        </span>
                                                    </td>
                                                    <td align="center" bgcolor="#FFFFFF" width="20" class="headingSpas" style="padding-left:5px; padding-right: 5px">
                                                        <span id ="imageButton${sno}" >
                                                            <img src="${pageContext.request.contextPath}/images/edit.gif" onclick="editRow('${billList.name}',${billList.percentage}, ${sno},'${billList.id}')" title="Edit"/>
                                                        </span>
                                                    </td>
                                                    <td align="center" bgcolor="#FFFFFF" width="25" class="headingSpas" style="padding-left:5px; padding-right: 5px">
                                                        <span id ="st${sno}" >
                                                            <a href="javascript:void(0)" onclick="cUDAction('${billList.status}', ${sno},'${billList.id}','${billList.name}')"  class="headingSpas" style="color: blue;" >
                                                                ${billList.status}&nbsp;
                                                            </a>
                                                        </span>
                                                    </td>
                        ...
                        <c:if test="${empty billList}">
                            <tr bgcolor="#FFFFFF">
                                <td valign="top" class="headingSpas" style="padding-top:10px;color: red">
                                    ...
                                    Bill Description '<%=request.getParameter("nameSrch") == null ? "" : request.getParameter("nameSrch")%>' not found
                                    <%}%>
                                </td>
                            </tr>
                        </c:if>
                                            ...
                                            <table width="100%" border="0" cellpadding="0" cellspacing="0" >
                                                <tr height="30" bgcolor="#FFFFFF">
                                                    ...
                                                    <td style="padding-left:10px;padding-top:5px" width="30%">
                                                        <input type="text" name="name"  id="Bill Description" class="headingSpas" size='200'  maxlength="200" style="width:170px" onblur="this.value=TrimAll(this.value)"/>
                                                    </td>
                                                    <td  style="padding-left:10px" width="20%" class="headingSpas" nowrap align="left">Discount%</td>
                                                    <td style="padding-left:10px;padding-top:5px" width="30%">
                                                        <input type="text" name="percentage"  id="percentage" class="headingSpas"  maxlength="6" style="width:170px" onblur="this.value=TrimAll(this.value)"/>
                                                    </td>
                                                ...
                                                <tr height="30" bgcolor="#FFFFFF">
                                                    <td colspan="4" style="padding-left:10px;padding-bottom: 5px" align="center" >
                                                        <input type=button name="add" class="headingSpas" value="Submit" onclick="cUDAction('add')"/>
                                                    </td>
                                                </tr>
                                            </table>

    ...
    function validate(){
      ...
      // alert(invDate1)
       // alert(receDate1)
        ...
        var tbObj= document.getElementById('myTb');
        var rowLength=tbObj.rows.length;
        for(var j=1;j<=rowLength-2;j++){
            if (document.getElementById("ReceivedQty"+j).value==""){
                document.getElementById("ReceivedQty"+j).focus();
                alert(not_blank_validation_msg+"Received Qty");
                return false;
            }
            if (!trim(document.getElementById("ReceivedQty"+j).value).match(/^[0-9]+$/)){
                document.getElementById("ReceivedQty"+j).focus();
                document.getElementById("ReceivedQty"+j).value="";
                alert(numeric_val_validation_msg+"Received Qty");
                return false;
            }
           if (eval(document.getElementById("ReceivedQty"+j).value) < 0 ){
                document.getElementById("ReceivedQty"+j).focus();
                document.getElementById("ReceivedQty"+j).value="";
                alert("Received Qty "+greater_than_val_msg+" or equals to 0");
                return false;
            }
            if (eval(document.getElementById("ReceivedQty"+j).value) > eval(document.getElementById("InvoiceQty"+j).value) ){
                document.getElementById("ReceivedQty"+j).focus();
                document.getElementById("ReceivedQty"+j).value="";
                alert("Received Qty "+gt_than_msg+" Invoice Qty");
                return false;
            }
        }
         document.getElementById("form2").submit();
         document.getElementById("grnSub").disabled=true;
    }
    function getInvoiceData(val){
       var strURL=contextPath + "/inventoryAction.do?option=getInvoiceData&invNo="+val+"&flag=part&t="+new Date().getTime();
        ...
        <div class="content_right1">
            <div class="con_slidediv2" style="position: relative;width: 100%">
              <h1 class="hText"><bean:message key="label.order.createGRN" /></h1>
                 <body>
                    <form action="<%=cntxpath%>/inventoryAction.do?option=createGRN" method="post" id="form1" onsubmit="return false" >
                        <input type="hidden" name="<%=org.apache.struts.taglib.html.Constants.TOKEN_KEY%>" value="<bean:write name="<%=org.apache.struts.Globals.TRANSACTION_TOKEN_KEY%>"/>">
                 ...
                 <c:if test="${not empty inventoryForm.grnPartList}">
                    <form action="<%=cntxpath%>/inventoryAction.do?option=addGRN" method="post" id="form2" onsubmit="return false" >
                        <input type="hidden" name="<%=org.apache.struts.taglib.html.Constants.TOKEN_KEY%>" value="<bean:write name="<%=org.apache.struts.Globals.TRANSACTION_TOKEN_KEY%>"/>">
                            <div class="content_right1">
                                    ...
                                    <table border=0 cellspacing=1 cellpadding=1  width=100%  bordercolor=#cccccc bgcolor=#cccccc id="myTb" name="myTb">
                                        ...
                                        <tr bgcolor="#eeeeee" height="20">
                                            <td colspan="6"  align="center" bgcolor="#FFFFFF" width="100%" style="padding-top:25px;">
                                                <input type="button" value="Submit" id="grnSub" class="headingSpas1" style="float:none;" onclick="validate()"/>
